...
function freshnessBody(iso: string): string {
  ...
  return JSON.stringify({
    freshnessInfo: { freshnesses: [{ aggregationPeriod: "DAILY", latestEndTime: { year, month, day } }] },
  });
}
...
describe("resolveVitalsWindow", () => {
  ...
  it("honors a custom day count", () => {
    expect(resolveVitalsWindow("2026-06-28", 7)).toEqual({ startDate: "2026-06-22", endDate: "2026-06-28" });
  });
...
describe("PlayReportingClient — auth + crash-rate query", () => {
  it("mints a reporting-scoped token, then POSTs a DAILY timeline and normalizes the rows", async () => {
    fetchMock
      ...
      .mockResolvedValueOnce(
        fakeResponse(
          200,
          JSON.stringify({
            rows: [crashRow("2026-06-02", "0.0123", "0.0099", "5000"), crashRow("2026-06-01", "0.02", "0.015", "4800")],
          }),
        ),
      );

    const rows = await client.queryCrashRate("com.example.app", { startDate: "2026-06-01", endDate: "2026-06-02" });
    ...
    expect(decodeJwtPayload(assertion)["scope"]).toBe("https://www.googleapis.com/auth/playdeveloperreporting");
    ...
    expect(rows).toEqual([
      { metric: "crash", date: "2026-06-01", rate: 0.02, userPerceivedRate: 0.015, distinctUsers: 4800 },
      { metric: "crash", date: "2026-06-02", rate: 0.0123, userPerceivedRate: 0.0099, distinctUsers: 5000 },
    ]);
  });
  ...
  it("pages through nextPageToken and reuses the cached token", async () => {
    fetchMock
      ...
      .mockResolvedValueOnce(
        fakeResponse(
          200,
          JSON.stringify({ rows: [crashRow("2026-06-01", "0.02", "0.01", "10")], nextPageToken: "p2" }),
        ),
      )
      .mockResolvedValueOnce(
    ...
    const secondPage = JSON.parse(fetchMock.mock.calls[2]![1].body as string) as { pageToken?: string };
  ...
  it("queries the ANR metric set with its own metric names", async () => {
    ...
    const rows = await client.queryAnrRate("com.example.app", { startDate: "2026-06-01", endDate: "2026-06-01" });
    ...
    expect(rows).toEqual([
      { metric: "anr", date: "2026-06-01", rate: 0.005, userPerceivedRate: 0.004, distinctUsers: 900 },
    ]);
  });
  ...
  it("omits missing metric columns rather than emitting NaN", async () => {
    fetchMock
      .mockResolvedValueOnce(fakeResponse(200, JSON.stringify({ access_token: "tok", expires_in: 3600 })))
      .mockResolvedValueOnce(
        fakeResponse(200, JSON.stringify({ rows: [{ startTime: { year: 2026, month: 6, day: 1 }, metrics: [] }] })),
      );

    const rows = await client.queryCrashRate("com.example.app", { startDate: "2026-06-01", endDate: "2026-06-01" });
  ...
  it("surfaces Google's error message on a failed query", async () => {
    fetchMock
      .mockResolvedValueOnce(fakeResponse(200, JSON.stringify({ access_token: "tok", expires_in: 3600 })))
      .mockResolvedValueOnce(fakeResponse(403, JSON.stringify({ error: { message: "Reporting API not enabled." } })));
