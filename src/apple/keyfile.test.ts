...
describe("extractKeyId", () => {
  it("pulls the Key ID out of an AuthKey_<KEYID>.p8 filename", () => {
    expect(extractKeyId("AuthKey_KEYABC1234.p8")).toBe("KEYABC1234");
  });

  it("works on a full path and upper-cases the result", () => {
    expect(extractKeyId("/Users/me/Downloads/AuthKey_keyxyz7890.p8")).toBe("KEYXYZ7890");
  });
...
describe("reconcileKeyId", () => {
  ...
  it("uses the filename id when nothing explicit was given", () => {
    expect(reconcileKeyId(undefined, "KEYABC1234")).toBe("KEYABC1234");
  });

  it("accepts a case-insensitive match between the two", () => {
    expect(reconcileKeyId("keyabc1234", "KEYABC1234")).toBe("KEYABC1234");
  });

  it("throws when an explicit Key ID contradicts the filename", () => {
    expect(() => reconcileKeyId("WRONGKEY12", "KEYABC1234")).toThrow(/doesn't match/);
  });
