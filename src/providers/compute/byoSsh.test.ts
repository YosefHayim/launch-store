...
describe("parseSshTarget", () => {
  it("parses user@host with the default port", () => {
    expect(parseSshTarget("ec2-user@1.2.3.4")).toEqual({ host: "1.2.3.4", user: "ec2-user", port: 22 });
  });
  ...
  it("defaults the user when only a host is given", () => {
    expect(parseSshTarget("my-mac.local")).toEqual({ host: "my-mac.local", user: "ec2-user", port: 22 });
  });
