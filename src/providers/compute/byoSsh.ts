...
import type { AllocateRequest, ComputeHost, HostHandle, HostStatus, SshTarget } from "../../core/types.js";
...
export function parseSshTarget(spec: string): SshTarget {
  ...
  if (!host) throw new Error(`Could not parse a host from "${spec}". Use user@host or user@host:port.`);
...
export const byoSshComputeHost: ComputeHost = {
  ...
  async allocate(request: AllocateRequest): Promise<HostHandle> {
    if (!request.sshTarget) throw new Error("byo-ssh needs an SSH target — pass `--remote user@host`.");
