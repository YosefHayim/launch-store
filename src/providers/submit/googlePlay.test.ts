...
// Capture the fastlane invocation instead of running it.
const runMock = vi.fn<(cmd: string, args: string[]) => Promise<void>>(() => Promise.resolve());
vi.mock("../../core/exec.js", () => ({ run: (cmd: string, args: string[]) => runMock(cmd, args) }));
...
function androidCtx(): ResolvedBuildContext {
  return {
    platform: "android",
    app: {
      name: "hello",
