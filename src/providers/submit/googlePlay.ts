...
import type { PlayTrack, ResolvedBuildContext, Submitter, SubmitTarget, BuildCredentials } from "../../core/types.js";
...
export const googlePlaySubmitter: Submitter = {
  ...
  async submit(
    ...
    if (creds.platform !== "android") throw new Error("The google-play submitter handles Android only.");
    const packageName = ctx.app.packageName;
    if (!packageName)
      throw new Error(`No Android application id for ${ctx.app.name}. Set android.package in app.json.`);

    // Resolved upstream onto ctx.android; fall back to the safe default for the neutral target.
    const track: PlayTrack = ctx.android?.track ?? (target === "production" ? "production" : "internal");
