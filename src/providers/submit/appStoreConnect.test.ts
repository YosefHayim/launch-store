...
const runMock = vi.fn<(cmd: string, args: string[], options?: { env?: Record<string, string> }) => Promise<void>>(() =>
  Promise.resolve(),
);
vi.mock("../../core/exec.js", () => ({
  run: (cmd: string, args: string[], options?: { env?: Record<string, string> }) => runMock(cmd, args, options),
}));
vi.mock("../../apple/apiKeyFile.js", () => ({ writeAscApiKeyFile: () => "/tmp/fake-asc-key.json" }));
...
function iosCtx(env: Record<string, string> = {}): ResolvedBuildContext {
  return {
    platform: "ios",
    app: { name: "hello", dir: "/tmp/hello", configPath: "/tmp/hello/app.json", bundleId: "com.example.hello" },
...
const IOS_CREDS: BuildCredentials = { platform: "ios", ascKey: { keyId: "K", issuerId: "I", p8: "PEM" } };
...
describe("app-store-connect submitter — binary upload via fastlane pilot", () => {
  it("uploads the ipa with pilot (review is now API-driven, never deliver) and forwards env", async () => {
    await appStoreConnectSubmitter.submit("/tmp/app.ipa", "production", IOS_CREDS, iosCtx({ FOO: "bar" }));
  ...
  it("rejects a non-iOS credential", async () => {
    ...
    await expect(appStoreConnectSubmitter.submit("/tmp/app.aab", "testing", androidCreds, iosCtx())).rejects.toThrow(
      /iOS only/,
    );
  });
});
