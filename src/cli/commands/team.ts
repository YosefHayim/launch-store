...
function renderInvitation(invitation: UserInvitationResource): string {
  ...
  const expires = invitation.expirationDate ? `expires ${invitation.expirationDate.slice(0, 10)}` : undefined;
  return [invitation.email, name, `[${invitation.roles.join(", ")}]`, expires].filter(Boolean).join("  ");
}

/** Render the whole team as a readable block. */
function renderTeam(team: Team): string {
  const lines = [`Team — ${team.members.length} member(s), ${team.invitations.length} pending invitation(s)`];
...
async function confirmWrite(message: string, yes: boolean | undefined): Promise<boolean> {
  ...
  if (!process.stdout.isTTY) {
    throw new Error("Refusing to change team access without confirmation. Re-run with --yes (non-interactive).");
  }
...
export function registerTeamCommand(program: Command): void {
  ...
  team
    ...
    .action(async (email: string, options: InviteOptions) => {
      ...
      if (!(await confirmWrite(`Invite ${email} to the team (sends them an email)?`, options.yes))) return;
  ...
  team
    ...
    .action(async (email: string, options: { yes?: boolean }) => {
      const log = createLogger(false);
      if (!(await confirmWrite(`Remove ${email} from the team (revoke access / cancel invitation)?`, options.yes))) {
        return;
      }
