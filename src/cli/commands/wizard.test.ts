...
const ANDROID_APP = app({ packageName: "com.example.sampleapp" });
...
describe("formatFlowSummary — the one-line repeat-build summary", () => {
  ...
  it("renders each iOS location with its wizard label", () => {
    ...
    expect(formatFlowSummary({ ...IOS_FLOW, location: "ssh", sshTarget: "ec2-user@host" })).toContain("Mac over SSH");
  ...
  it("omits the location for Android and says 'build only' when not uploading", () => {
    const flow: LastFlow = { platform: "android", location: "local", profile: "production", submit: false };
...
describe("flowInvalidReason — gates whether a repeat is offered", () => {
  ...
  it("returns null for a resolvable Android flow (no account needed)", () => {
    const flow: LastFlow = { platform: "android", location: "local", profile: "production", submit: true };
    expect(flowInvalidReason(flow, configWith(["production"]), [ANDROID_APP], new Set())).toBeNull();
  });
  ...
  it("rejects an SSH flow that lost its target", () => {
    const flow: LastFlow = { platform: "ios", location: "ssh", profile: "production", submit: true, account: "ABC123" };
