...
export function registerCompletionCommand(program: Command): void {
  ...
  completion
    ...
    .action((options: { shell?: string }) => {
      ...
      const result = installCompletion(
        options.shell !== undefined ? { shell: parseShell(options.shell) } : {},
      );
      if (result.status === 'manual') {
        log.warn(`Could not edit ${result.rcFile} — its directory doesn't exist yet.`);
        log.tip(
          `Add this line to your ${result.shell} config, then restart your shell: ${result.line}`,
        );
        return;
      }
      log.step(
        'completion',
        `${result.updated ? 'updated' : 'installed'} for ${result.shell} in ${result.rcFile}`,
      );
  ...
  completion
    ...
    .action((shell: string | undefined) => {
      ...
      if (!resolved) {
        throw new Error(
          `Could not detect your shell. Pass one explicitly: launch completion ${SHELLS.join('|')}.`,
        );
      }
