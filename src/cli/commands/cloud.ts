...
async function status(): Promise<void> {
  ...
  console.log(`  releasable after ${new Date(live.releasableAt).toLocaleString()} (24h Apple-license minimum).`);
}
...
async function setup(): Promise<void> {
  ...
  console.log(`AWS region: ${config.aws.region}${config.aws.profile ? ` · profile ${config.aws.profile}` : ""}`);
  console.log(
    ...
    .action(async (action: string) => {
      switch (action) {
        ...
        case "setup":
          ...
          return;
        default:
          throw new Error(`Unknown action "${action}". Use "setup", "status", "teardown", or "doctor".`);
      }
    });
}
