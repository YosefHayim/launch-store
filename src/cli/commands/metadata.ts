 ...
 */
export async function pullAppleListing(
  bundleId: string,
  configPath: string,
  dryRun: boolean,
): Promise<void> {
  ...
  try {
    if (dryRun) {
      log.step(
        'metadata',
        `would run \`fastlane deliver download_metadata\` for ${bundleId} → ${workDir}`,
      );
      return;
    }
    await run('fastlane', [
...
async function pushApple(bundleId: string, configPath: string, dryRun: boolean): Promise<void> {
  ...
  if (dryRun) {
    log.step(
      'metadata',
      `would push ${written.length} App Store field(s) for ${bundleId} via \`fastlane deliver\``,
    );
...
async function pushAndroid(
  ...
  if (dryRun) {
    log.step(
      'metadata',
      `would push ${written.length} Play field(s) for ${packageName} via \`fastlane supply\``,
    );
    log.info(`rehearsed into ${workDir} (no upload)`);
    return;
  }
  ...
  metadata
    ...
    .action(async (options: MetadataOptions) => {
      ...
      if (isApplePlatform(platform)) {
        if (!target.bundleId)
          throw new Error('No bundle identifier for this app (set ios.bundleIdentifier).');
        await pullAppleListing(target.bundleId, target.configPath, options.dryRun === true);
      } else {
        if (!target.packageName)
          throw new Error('No Android application id for this app (set android.package).');
