...
describe('formatStatusLine', () => {
  ...
  it('annotates a build still processing', () => {
    expect(formatStatusLine(status({ buildProcessingState: 'PROCESSING' }))).toContain(
      'build 42 (PROCESSING)',
    );
  });
  ...
  it('shows the phased-rollout state and a live verdict', () => {
    const line = formatStatusLine(
      status({ appStoreState: 'READY_FOR_SALE', phasedReleaseState: 'ACTIVE' }),
    );
  ...
  it('handles an app with no App Store version yet', () => {
    expect(
      formatStatusLine(status({ versionString: null, appStoreState: '', buildNumber: null })),
