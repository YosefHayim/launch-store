...
describe("renderAction", () => {
  it("marks a change with +, a skip with •", () => {
    expect(
      renderAction({ description: "create achievement first_win (10 pts)", destructive: false, status: "planned" }),
    ).toBe("+ create achievement first_win (10 pts)");
    expect(renderAction({ description: "enable Game Center for the app", destructive: false, status: "applied" })).toBe(
