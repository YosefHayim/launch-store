...
function selectApps(apps: AppDescriptor[], selector: string | undefined): AppDescriptor[] {
  ...
  return wanted.map((name) => {
    ...
    if (!app) throw new Error(`Unknown app "${name}". Discovered: ${apps.map((a) => a.name).join(", ") || "none"}.`);
    return app;
  });
}
...
function pickApp(apps: AppDescriptor[], selector: string | undefined): AppDescriptor {
  ...
  if (selector) {
    ...
    if (!match) throw new Error(`Unknown app "${selector}". Discovered: ${withBundle.map((a) => a.name).join(", ")}.`);
    return match;
  }
  if (withBundle.length === 1 && withBundle[0]) return withBundle[0];
  throw new Error(`Several apps found — pass --app <name> (one of: ${withBundle.map((a) => a.name).join(", ")}).`);
}

/** One app's reconcile outcome — its job plus the report, or a precondition error that skipped it. */
type JobOutcome = { app: AppDescriptor; report: ReconcileReport } | { app: AppDescriptor; error: string };
...
async function runReconcile(options: OffersOptions): Promise<void> {
  ...
  if (jobs.length === 0) {
    log.info("Nothing to reconcile — no app declares offers or promoted purchases. Add them under `products`.");
    return;
  }
  ...
  for (const plan of plans) {
    ...
    log.notice(
      ...
      ...actions.map((action) => (action.status === "skipped" ? `• ${action.description}` : `+ ${action.description}`)),
    );
  }
  ...
  if (options.yes !== true) {
    ...
    const proceed = await confirm({ message: `Apply ${mutationCount} offer change(s) to App Store Connect?` });
  ...
  const toApply = plans.flatMap((plan) =>
    "report" in plan && plan.report.actions.some((action) => action.status === "planned") ? [plan] : [],
  );
...
async function resolveSubscription(
  ...
  for (const group of await client.listSubscriptionGroups(appId)) {
    const sub = (await client.listSubscriptions(group.id)).find((entry) => entry.productId === productId);
...
export function registerOffersCommand(program: Command): void {
  const offers = program
    .command("offers")
    .description("reconcile subscription offers (codes, promo/intro/win-back) and promoted-purchase order from config")
    .option("-a, --app <names>", "comma-separated app handles (default: all apps with offers declared)")
  ...
  offers
    ...
    .option("--custom <code>", "create one shareable custom code with this value instead of one-time-use codes")
    .action(
      async (
        productId: string,
        offerName: string,
        ...
        if (!code) throw new Error(`No offer code named "${offerName}" on ${productId} (${appName}).`);

        if (options.custom !== undefined) {
          await client.createOfferCodeCustomCode(code.id, options.custom, count, options.expires);
          log.step("offers", `created custom code "${options.custom}" (${count} uses) on "${offerName}"`);
          return;
        }
        if (!options.expires) throw new Error("One-time-use codes need an expiration date: --expires YYYY-MM-DD.");
        await client.createOfferCodeOneTimeUseBatch(code.id, count, options.expires);
        log.step("offers", `generated ${count} one-time-use code(s) on "${offerName}" (expire ${options.expires})`);
      },
    );
  ...
  offers
    ...
    .action(async (productId: string, options: AppScopedOptions) => {
      ...
      log.notice(
        ...
        ...codes.map((code) => `${code.active ? "●" : "○"} ${code.name}${code.active ? "" : " (inactive)"}`),
      );
    });
  ...
  offers
    ...
    .action(async (productId: string, offerName: string, options: AppScopedOptions) => {
      const log = createLogger(false);
      const { client, appName, subscriptionId } = await resolveSubscription(options, productId);
      const code = (await client.listSubscriptionOfferCodes(subscriptionId)).find((entry) => entry.name === offerName);
      if (!code) throw new Error(`No offer code named "${offerName}" on ${productId} (${appName}).`);
