...
export function parsePrice(amount: string, currency: string): PlayMoneyUnits {
  ...
  if (!AMOUNT_PATTERN.test(trimmed)) {
    throw new Error(`<amount> must be a non-negative decimal with up to 9 places (got "${amount}").`);
  }
...
function renderPrices(base: PlayMoneyUnits, converted: ConvertedPrices): string {
  ...
  const lines = converted.regions.map((region) => `  ${region.regionCode.padEnd(4)}${formatMoney(region.price)}`);
