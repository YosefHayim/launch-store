...
import { listSecretRefs, removeBuildSecret, setBuildSecret, type SecretRef } from "../../core/buildSecrets.js";
...
async function askValue(name: string): Promise<string> {
  const value = await password({ message: `Value for ${name}`, validate: (v) => (v === "" ? "Required." : undefined) });
  if (isCancel(value)) {
    cancel("Cancelled.");
    process.exit(0);
  }
...
async function setSecretValue(name: string | undefined, options: SecretOptions): Promise<void> {
  if (!name) throw new Error("Usage: launch secret set <NAME> [--value <v>] [--app <app>] [--profile <profile>].");
  if (!ENV_NAME.test(name)) throw new Error(`"${name}" is not a valid env var name (letters, digits, underscores).`);
  const app = await resolveApp(options);
  const canPrompt = options.yes !== true && process.stdin.isTTY;
  let value = options.value;
  if (value === undefined) {
    if (!canPrompt) throw new Error("A value is required. Pass --value <v> (or run in an interactive terminal).");
...
export function registerSecretCommand(program: Command): void {
  program
    ...
    .option("--value <value>", "set: the secret value (else prompted; required when non-interactive)")
