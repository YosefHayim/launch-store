...
import type { BuildArtifact, PrunedArtifact } from '@core/types/artifacts.js';
const MB = 1024 * 1024;
...
describe('toBuildRow', () => {
  it('derives the worst-case download from the per-device entries', () => {
    ...
    expect(tableRow33.downloadBytes).toBe(47 * MB);
    expect(tableRow33.artifactBytes).toBe(61 * MB);
    expect(tableRow33.id).toBe('demo-1.0.0-7-ios');
  });
});
...
describe('formatBuildsTable', () => {
  it('renders a header plus one aligned row per build', () => {
    ...
    const [header, tableRow88] = table.split('\n');
    expect(header).toContain('BUILD');
    expect(header).toContain('DOWNLOAD');
    expect(header).toContain('TYPE');
    expect(tableRow88).toContain('demo');
    expect(tableRow88).toContain('38.0 MB');
    expect(tableRow88).toContain('2026-06-13 14:02');
    expect(tableRow88).toContain('clean');
  });
