...
export function registerReleaseConfigCommand(program: Command): void {
  program
    ...
    .action(async (options: ReleaseConfigOptions) => {
      ...
      if (planned.length === 0) {
        log.gap();
        log.step("release-config", "nothing to apply (everything in sync; skipped areas need a version first)");
        return;
      }
      ...
      if (options.yes !== true) {
        ...
        const proceed = await confirm({ message: `Apply ${planned.length} change(s) to App Store Connect?` });
