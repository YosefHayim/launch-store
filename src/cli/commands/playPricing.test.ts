...
describe('parsePrice', () => {
  it('splits a decimal amount into units + nanos and upper-cases the currency', () => {
    expect(parsePrice('4.99', 'usd')).toEqual({
      currencyCode: 'USD',
      units: '4',
      nanos: 990_000_000,
    });
  ...
  it('pads the fraction to nanos precision (one decimal place)', () => {
    expect(parsePrice('4.5', 'USD')).toEqual({
      currencyCode: 'USD',
      units: '4',
      nanos: 500_000_000,
    });
  ...
  it('strips leading zeros from the whole part', () => {
    expect(parsePrice('007.50', 'EUR')).toEqual({
      currencyCode: 'EUR',
      units: '7',
      nanos: 500_000_000,
    });
...
describe('formatMoney', () => {
  ...
  it('keeps full precision for many-decimal nanos', () => {
    expect(formatMoney({ currencyCode: 'USD', units: '4', nanos: 123_456_789 })).toBe(
      'USD 4.123456789',
    );
  });
});
