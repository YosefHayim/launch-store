...
describe("resolveCompletions (the __complete callback)", () => {
  ...
  function program(): Command {
    ...
    root.command("build").option("-p, --profile <name>", "build profile").option("--clean", "force clean");
  ...
  it("resolves -a/--app values from discovered apps", async () => {
    ...
    writeFileSync(join(repo, "apps", "atlas", "app.json"), JSON.stringify({ expo: { slug: "atlas" } }));
    const saved = process.cwd();
    process.chdir(repo);
    try {
