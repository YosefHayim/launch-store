  ...
  async check(ctx: ReadinessContext): Promise<ProbeResult> {
    ...
    const results: AppReadiness[] = await Promise.all(
      apps.map(async ({ name, identifier }) => {
        ...
        if (!required) {
          return { app: name, identifier, status: "ok" as const, detail: "no sign-in required for App Review" };
        }
        return hasName
          ? { app: name, identifier, status: "ok" as const, detail: "demo account provided for App Review" }
          : {
              app: name,
              identifier,
