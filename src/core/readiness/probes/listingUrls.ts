...
async function fetchStatus(url: string): Promise<number> {
  const response = await fetch(url, { method: "GET", signal: AbortSignal.timeout(URL_LIVENESS_TIMEOUT_MS) });
    ...
    const results: AppReadiness[] = await Promise.all(
      urls.map(async ({ app, field, url }) => {
        ...
        return status >= 200 && status < 300
          ? { app, identifier: url, status: "ok" as const, detail: `${field} URL live (HTTP ${status})` }
          : {
              app,
              identifier: url,
