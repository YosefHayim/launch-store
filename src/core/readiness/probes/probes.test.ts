...
function ascApi(over: Partial<AscReadinessApi> = {}): AscReadinessApi {
  return {
    ...
    listDistributionCertificates: vi.fn(async () => [
      { id: 'c1', expirationDate: '2099-01-01T00:00:00Z' },
    ]),
    ...
    getAgeRatingDeclaration: vi.fn(async () => ({
      attributes: { violenceCartoonOrFantasy: false },
    })),
    listAccountDeletionUrls: vi.fn(async () => [
      { locale: 'en-US', url: 'https://x.example/delete' },
    ]),
    ...
    listScreenshotSets: vi.fn(async () => [
      { id: 'set-1', screenshotDisplayType: 'APP_IPHONE_67' },
    ]),
...
function findings(result: ProbeResult): { status: string; identifier: string }[] {
  return result.state === 'checked'
    ? result.apps.map(({ status, identifier }) => ({ status, identifier }))
    : [];
}
...
describe('iapProductsProbe', () => {
  ...
  it('passes a present product, blocks a missing one and a MISSING_METADATA one', async () => {
    const present = ascApi({
      listInAppPurchases: vi.fn(async () => [
        { productId: 'com.x.coins', state: 'READY_TO_SUBMIT' },
      ]),
    });
    expect(
      findings(
    ...
    const incomplete = ascApi({
      listInAppPurchases: vi.fn(async () => [
        { productId: 'com.x.coins', state: 'MISSING_METADATA' },
      ]),
    });
    expect(
      findings(
...
describe('accountDeletionProbe', () => {
  ...
  it('passes when a URL is declared in any locale, warns when none is set', async () => {
    ...
    const none = ascApi({
      listAccountDeletionUrls: vi.fn(async () => [{ locale: 'en-US', url: '' }]),
    });
    expect(
      findings(
...
describe('screenshotsProbe', () => {
  ...
  it('warns when iPhone screenshots exist but not for the required class, blocks when none exist', async () => {
    const otherClass = ascApi({
      listScreenshotSets: vi.fn(async () => [
        { id: 'set-1', screenshotDisplayType: 'APP_IPHONE_65' },
      ]),
    });
    expect(
      findings(
