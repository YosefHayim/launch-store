...
function app(overrides: Partial<AppDescriptor> = {}): AppDescriptor {
  return {
    name: "sampleapp",
    dir: "/repo/sampleapp",
    configPath: "/repo/sampleapp/app.json",
...
describe("previewBuild", () => {
  ...
  it("rehearses an internal upload as the internal testing track", () => {
    const preview = previewBuild({ config: config(), apps: [app()], platform: "android", distribution: "internal" });
  ...
  it("honors a profile's track and rollout defaults", () => {
    const preview = previewBuild({ config: config(), apps: [app()], platform: "android", profile: "preview" });
  ...
  it("throws on an explicit unknown profile rather than silently defaulting", () => {
    expect(() => previewBuild({ config: config(), apps: [app()], platform: "ios", profile: "ghost" })).toThrow(
      'Unknown profile "ghost"',
    );
  });

  it("falls back to production, then the first profile, when no profile is given", () => {
    expect(previewBuild({ config: config(), apps: [app()], platform: "ios" }).profile).toBe("production");
    const noProd = config({ profiles: { staging: { name: "staging" } } });
    expect(previewBuild({ config: noProd, apps: [app()], platform: "ios" }).profile).toBe("staging");
  });
});
