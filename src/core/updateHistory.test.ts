...
import { historySnapshotKey, manifestKey, manifestSignatureKey, rollbackDirectiveKey } from "./otaManifest.js";
import {
...
function manifest(id: string): UpdateManifest {
  return {
    ...
    createdAt: "2026-06-13T00:00:00.000Z",
    runtimeVersion: "1.0.0",
    launchAsset: { key: "bundle", contentType: "application/javascript", url: "https://cdn/bundle.hbc" },
    assets: [{ key: "logo", contentType: "image/png", url: "https://cdn/logo.png", fileExtension: ".png" }],
    metadata: {},
    extra: {},
  };
}
...
describe("recordPublish", () => {
  ...
  it("keeps a different runtime version's active update untouched", async () => {
    const storage = fakeStorage();
    await recordPublish(storage, "production", "ios", entry({ id: "rtv1", runtimeVersion: "1.0.0" }));
    await recordPublish(storage, "production", "ios", entry({ id: "rtv2", runtimeVersion: "2.0.0" }));
    const history = await readHistory(storage, "production", "ios");
...
describe("republishUpdate", () => {
  it("writes a fresh active manifest + snapshot + signed sig and records a rollback entry", async () => {
    ...
    const active = JSON.parse(storage.objects.get(manifestKey(channel, platform, "1.0.0"))!) as UpdateManifest;
    expect(active.id).toBe("rollback-id");
    expect(storage.objects.has(historySnapshotKey(channel, platform, "1.0.0", "rollback-id"))).toBe(true);
    expect(storage.objects.get(manifestSignatureKey(channel, platform, "1.0.0"))).toContain('sig="FAKE"');
...
describe("rollback directive", () => {
  it("writes an active, signed rollBackToEmbedded directive served verbatim", async () => {
    ...
    const stored = JSON.parse(storage.objects.get(rollbackDirectiveKey("production", "ios", "1.0.0"))!);
  ...
  it("clearRollbackDirective deactivates an active directive and no-ops when absent", async () => {
    ...
    const cleared = JSON.parse(storage.objects.get(rollbackDirectiveKey("production", "ios", "1.0.0"))!);
