...
const CONFIG: VersionExperimentsConfig = {
  experiments: [
    { name: "Icon Test", trafficProportion: 50, treatments: [{ name: "Variant A" }, { name: "Variant B" }] },
  ],
};
...
describe("parseVersionExperimentsConfig", () => {
  it("parses experiments with treatments", () => {
    ...
    expect(config.experiments[0]?.treatments?.map((treatment) => treatment.name)).toEqual(["Variant A", "Variant B"]);
  });
  ...
  it("rejects a non-object, an empty list, a bad traffic proportion, and a duplicate name", () => {
    ...
    expect(() => parseVersionExperimentsConfig({ experiments: [{ name: "X", trafficProportion: 0 }] })).toThrow(
      /trafficProportion must be a positive number/,
    );
    expect(() =>
      parseVersionExperimentsConfig({
        experiments: [
...
describe("reconcileVersionExperiments", () => {
  ...
  it("creates a missing experiment and each treatment (apply)", async () => {
    ...
    const report = await reconcileVersionExperiments(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: false });
    expect(calls.createdExperiments).toEqual([{ name: "Icon Test", platform: "IOS", trafficProportion: 50 }]);
  ...
  it("only creates treatments the existing experiment doesn't already have", async () => {
    ...
    await reconcileVersionExperiments(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: false });
  ...
  it("plans but performs nothing on a dry-run", async () => {
    const { api, calls } = makeApi({});
    const report = await reconcileVersionExperiments(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: true });
  ...
  it("skips treatments when the experiment create failed", async () => {
    ...
    expect(report.actions.find((action) => action.status === "failed")?.error).toBe("name already in use");
  });
});
