...
describe("autoUpgradeBlockedReason", () => {
  ...
  it("blocks the re-exec child (loop guard)", () => {
    expect(autoUpgradeBlockedReason({ ...base, env: { LAUNCH_UPGRADED: "1" } })).toMatch(/loop guard/);
  });
  it("blocks on explicit opt-out, CI, non-TTY, and dev (source) runs", () => {
    expect(autoUpgradeBlockedReason({ ...base, env: { LAUNCH_NO_UPGRADE: "1" } })).toMatch(/LAUNCH_NO_UPGRADE/);
    ...
    expect(autoUpgradeBlockedReason({ ...base, scriptPath: "/repo/src/cli/index.ts" })).toMatch(/dev/);
  });
});
...
function makeDeps(overrides: Partial<UpdateCheckDeps> = {}): {
  ...
  const calls = { fetch: 0, upgrade: 0, reexec: 0, writes: [] as UpdateState[], notes: [] as string[] };
