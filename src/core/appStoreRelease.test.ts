...
function makeApi(overrides: Partial<AscReleaseApi> = {}): AscReleaseApi {
  const base: AscReleaseApi = {
    ...
    createAppStoreVersionLocalization: vi.fn().mockResolvedValue({ id: "loc-new", locale: "en-US" }),
...
const VALID_BUILD: BuildResource = { id: "b-1", version: "42", processingState: "VALID", expired: false };
...
describe("classifyVerdict — the --watch / exit-code contract", () => {
  it("maps states to verdicts with the documented exit codes", () => {
    expect(classifyVerdict("READY_FOR_SALE")).toMatchObject({ state: "released", done: true, exitCode: 0 });
    expect(classifyVerdict("PENDING_DEVELOPER_RELEASE")).toMatchObject({
      state: "pending-release",
      done: true,
      exitCode: 0,
    });
    expect(classifyVerdict("IN_REVIEW")).toMatchObject({ state: "in-review", done: false, exitCode: 3 });
    expect(classifyVerdict("WAITING_FOR_REVIEW")).toMatchObject({ done: false, exitCode: 3 });
    expect(classifyVerdict("REJECTED")).toMatchObject({ state: "rejected", done: true, exitCode: 2 });
...
describe("releaseApp — submit an update over the API", () => {
  it("creates the version, attaches the build, declares compliance, writes notes, and submits", async () => {
    ...
    expect(api.createAppStoreVersion).toHaveBeenCalledWith(
      "app1",
      expect.objectContaining({ versionString: "1.2.0", platform: IOS_PLATFORM, releaseType: "AFTER_APPROVAL" }),
    );
    ...
    expect(api.updateAppStoreVersion).toHaveBeenCalledWith("v-new", { releaseType: "AFTER_APPROVAL" });
    ...
    expect(report).toMatchObject({ submitted: true, alreadyInReview: false, appStoreState: "WAITING_FOR_REVIEW" });
  });
  ...
  it("updates an existing localization's notes instead of creating a duplicate", async () => {
    const api = makeApi({
      ...
      listAppStoreVersionLocalizations: vi.fn().mockResolvedValue([{ id: "loc-1", locale: "en-US", whatsNew: "old" }]),
    });
  ...
  it("retargets the open editable version when its version string differs (one editable allowed at a time)", async () => {
    const api = makeApi({
      listAppStoreVersions: vi
        .fn()
        .mockResolvedValue([{ id: "v5", versionString: "1.1.0", appStoreState: "PREPARE_FOR_SUBMISSION" }]),
    });
  ...
  it("is an idempotent no-op when the version is already in review", async () => {
    const api = makeApi({
      listAppStoreVersions: vi
        .fn()
        .mockResolvedValue([{ id: "v9", versionString: "1.2.0", appStoreState: "WAITING_FOR_REVIEW" }]),
    });
    const report = await releaseApp(api, input());
    expect(report).toMatchObject({ submitted: false, alreadyInReview: true, appStoreState: "WAITING_FOR_REVIEW" });
  ...
  it("errors with the portal checklist when the app has no App Store Connect record", async () => {
    ...
    await expect(releaseApp(api, input())).rejects.toThrow(/No App Store Connect app record.*Apple has no API/s);
  });
...
describe("waitForValidBuild", () => {
  ...
  it("returns the build once it reaches VALID", async () => {
    ...
    await expect(waitForValidBuild(api, "app1", 42, { sleep: noSleep })).resolves.toEqual(VALID_BUILD);
  });

  it("polls until VALID, sleeping between attempts", async () => {
    const findBuildByVersion = vi
      .fn()
      .mockResolvedValueOnce({ id: "b", version: "42", processingState: "PROCESSING", expired: false })
  ...
  it("throws on timeout", async () => {
    const api = makeApi({
      findBuildByVersion: vi
        .fn()
        .mockResolvedValue({ id: "b", version: "42", processingState: "PROCESSING", expired: false }),
    });
    await expect(waitForValidBuild(api, "app1", 42, { sleep: noSleep, timeoutMs: 0 })).rejects.toThrow(
      /still PROCESSING/,
    );
  });
});
