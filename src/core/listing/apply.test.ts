...
import { applyDraft, briefFor, clampDraft, deriveAndroidLocale, renderDraftPreview } from "./apply.js";
...
describe("briefFor", () => {
  ...
  it("leaves optional fields absent when there's no seed", () => {
    expect(briefFor("en-US", "MyApp", undefined, undefined)).toEqual({ locale: "en-US", appName: "MyApp" });
  });
});

describe("deriveAndroidLocale", () => {
  it("maps title/description across and borrows the subtitle for the short description", () => {
    const android = deriveAndroidLocale({ title: "MyApp", subtitle: "Stay on task", description: "Long copy." });
    expect(android).toEqual({ title: "MyApp", shortDescription: "Stay on task", fullDescription: "Long copy." });
  });
...
describe("applyDraft", () => {
  ...
  it("merges into the targeted locale, preserving other fields and other locales", () => {
    ...
    expect(next.apple?.info["en-US"]).toEqual({ subtitle: "Keep me", title: "New Title", keywords: ["focus"] });
  ...
  it("writes the derived android section when android is targeted", () => {
    const next = applyDraft({}, "en-US", { title: "MyApp", description: "Long." }, { ios: false, android: true });
...
describe("renderDraftPreview", () => {
  it("shows iOS fields with character budgets and surfaces warnings", () => {
    const preview = renderDraftPreview(
      [{ locale: "en-US", draft: { title: "MyApp", keywords: ["a", "b"] }, warnings: ["title was trimmed"] }],
