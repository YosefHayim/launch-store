...
export function clampDraft(draft: DraftListing): { draft: DraftListing; warnings: string[] } {
  ...
  const promotionalText = fit(draft.promotionalText, APPLE_LIMITS.promotionalText, "promotional text");
...
export function deriveAndroidLocale(draft: DraftListing): AndroidLocaleInfo {
  ...
  if (short !== undefined) info.shortDescription = clampText(short, ANDROID_LIMITS.shortDescription);
...
export function applyDraft(
  ...
  if (targets.android) {
    ...
    next.android = {
      ...android,
      info: { ...android.info, [locale]: { ...android.info[locale], ...deriveAndroidLocale(draft) } },
    };
  }
  return next;
}
 ...
 */
export function renderDraftPreview(drafts: LocaleDraft[], targets: { ios: boolean; android: boolean }): string {
  const blocks = drafts.map(({ locale, draft, warnings }) => {
    const lines = [`  ${locale}`];
    if (targets.ios) {
      if (draft.title !== undefined) lines.push(row("title", draft.title, draft.title.length, APPLE_LIMITS.title));
      ...
      if (draft.promotionalText !== undefined)
        lines.push(
          row("promo text", draft.promotionalText, draft.promotionalText.length, APPLE_LIMITS.promotionalText),
        );
      if (draft.description !== undefined)
        lines.push(row("description", draft.description, draft.description.length, APPLE_LIMITS.description));
    }
    if (targets.android) {
      ...
      if (android.shortDescription !== undefined)
        lines.push(
          row("short desc", android.shortDescription, android.shortDescription.length, ANDROID_LIMITS.shortDescription),
        );
      if (android.fullDescription !== undefined)
        lines.push(
          row("full desc", android.fullDescription, android.fullDescription.length, ANDROID_LIMITS.fullDescription),
        );
    }
