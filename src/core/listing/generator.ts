...
export function buildListingPrompt(brief: ListingBrief): string {
  ...
  if (brief.current && Object.keys(brief.current).length > 0) {
    lines.push("", "Improve on the current listing (do not copy it verbatim):", JSON.stringify(brief.current, null, 2));
  }
  return lines.join("\n");
}
...
export function parseDraftListing(text: string): DraftListing {
  ...
  if (Object.keys(draft).length === 0) throw new Error("The model returned no usable listing fields.");
  return draft;
}
...
function extractText(payload: unknown): string {
  ...
  if (!Array.isArray(content)) throw new Error("Unexpected Anthropic response: missing content array.");
  const text = content
 ...
 */
export function createAnthropicListingGenerator(options: { model?: string; apiKey?: string } = {}): ListingGenerator {
