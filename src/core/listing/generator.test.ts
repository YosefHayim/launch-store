...
import { buildListingPrompt, createAnthropicListingGenerator, parseDraftListing } from "./generator.js";
...
describe("createAnthropicListingGenerator", () => {
  ...
  it("labels itself with the resolved model", () => {
    expect(createAnthropicListingGenerator({ model: "claude-test" }).name).toBe("anthropic:claude-test");
  });

  it("throws an actionable error when no API key is available", async () => {
    const generator = createAnthropicListingGenerator();
    await expect(generator.generate({ locale: "en-US", appName: "MyApp" })).rejects.toThrow(/ANTHROPIC_API_KEY/);
  });
});
