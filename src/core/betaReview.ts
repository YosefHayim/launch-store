...
function selectBuild(builds: BuildResource[], buildVersion: string | undefined): BuildResource {
  if (buildVersion) {
    ...
    if (!match) {
      throw new Error(`No build ${buildVersion} for this app. Upload it first, or omit --build to use the latest.`);
    }
    if (match.expired) {
      throw new Error(`Build ${buildVersion} has expired (TestFlight's 90-day limit) and can't be submitted.`);
    }
    return match;
  }
  const latest = builds.find((build) => build.processingState === "VALID" && !build.expired);
  if (!latest) {
    throw new Error("No VALID, non-expired build to release. Upload a build and wait for processing to finish.");
  }
  return latest;
}
...
export async function reconcileBetaReview(
  ...
  const build = selectBuild(await api.listBuilds(input.appId, BUILD_SCAN_LIMIT), input.buildVersion);
...
async function reconcileNotes(
  ...
  const existing = new Map(
    (await api.listBetaBuildLocalizations(buildId)).map((localization) => [localization.locale, localization]),
  );
  ...
  for (const [locale, text] of Object.entries(whatToTest)) {
    ...
    const action = plan(ctx, current ? `update "What to Test" (${locale})` : `set "What to Test" (${locale})`);
...
async function reconcileSubmission(ctx: ReconcileContext, api: AscBetaReviewApi, buildId: string): Promise<void> {
  const existing = await api.getBetaAppReviewSubmission(buildId);
  if (existing) {
    skip(ctx, `submit for Beta App Review: build already submitted (${describeState(existing.state)})`);
    return;
  }
...
export function parseBetaReviewConfig(raw: unknown): BetaReviewConfig {
  ...
  if (!whatToTest) {
    throw new Error('testflight.config.json: "whatToTest" must be an object mapping locale → notes.');
  }

  const notes: Record<string, string> = {};
  for (const [locale, value] of Object.entries(whatToTest)) {
    if (typeof value !== "string" || value.length === 0) {
      throw new Error(`testflight.config.json: whatToTest["${locale}"] must be a non-empty string.`);
    }
    notes[locale] = value;
  }
...
export function loadBetaReviewConfig(path: string): BetaReviewConfig {
  if (!existsSync(path)) {
    throw new Error(`No TestFlight config at ${path}. Add a "whatToTest" map, or pass --whats-new <text>.`);
  }
...
export function summarizeBetaReview(actions: PlannedAction[]): { applied: number; failed: number; skipped: number } {
  let applied = 0;
  let failed = 0;
  let skipped = 0;
