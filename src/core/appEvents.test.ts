...
import { createEvent, deleteEvent, listEvents, localizeEvent, type AscAppEventsApi } from "./appEvents.js";
...
function makeApi(overrides: Partial<AscAppEventsApi> = {}): AscAppEventsApi {
  const base: AscAppEventsApi = {
    ...
    createAppEventLocalization: vi
      .fn()
      .mockImplementation((_eventId: string, locale: string, attributes: AppEventLocalizationInput) =>
...
describe("listEvents", () => {
  it("pairs each event with its localizations", async () => {
    const api = makeApi({
      ...
      listAppEventLocalizations: vi.fn().mockResolvedValue([{ id: "l1", locale: "en-US", name: "Summer Sale" }]),
    });
  ...
  it("throws an actionable error when the app has no App Store Connect record", async () => {
    ...
    await expect(listEvents(api, "com.acme.app")).rejects.toThrow(/No App Store Connect app record/);
  });
});
...
describe("createEvent", () => {
  ...
  it("rejects an unknown badge with the valid list", async () => {
    await expect(createEvent(makeApi(), "com.acme.app", { referenceName: "X", badge: "MEGA_EVENT" })).rejects.toThrow(
      /Invalid badge "MEGA_EVENT"/,
    );
  });

  it("rejects an unknown priority", async () => {
    await expect(createEvent(makeApi(), "com.acme.app", { referenceName: "X", priority: "URGENT" })).rejects.toThrow(
      /Invalid priority/,
    );
  });

  it("rejects an unknown purpose", async () => {
    await expect(createEvent(makeApi(), "com.acme.app", { referenceName: "X", purpose: "FOR_FUN" })).rejects.toThrow(
      /Invalid purpose/,
    );
  });
...
describe("localizeEvent", () => {
  ...
  it("updates the existing localization (case-insensitive) and carries the locale through", async () => {
    const api = makeApi({
      listAppEventLocalizations: vi.fn().mockResolvedValue([{ id: "l9", locale: "en-US", name: "Old" }]),
    });
  ...
  it("rejects an empty locale", async () => {
    await expect(localizeEvent(makeApi(), "e1", { locale: "  ", name: "x" })).rejects.toThrow(/locale is required/);
  });

  it("rejects when no copy fields are provided", async () => {
    await expect(localizeEvent(makeApi(), "e1", { locale: "en-US" })).rejects.toThrow(/at least one of/);
  });
});
