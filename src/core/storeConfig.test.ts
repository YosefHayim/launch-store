...
describe("parseStoreConfig", () => {
  it("parses the Expo apple schema plus the android extension", () => {
    ...
    expect(config.apple?.info["en-US"]).toEqual({ title: "Hello", keywords: ["a", "b"], description: "Desc" });
  ...
  it("rejects a document with neither platform section", () => {
    expect(() => parseStoreConfig({ configVersion: 0 })).toThrow(/neither an "apple" nor an "android"/);
  });

  it("drops undefined/non-string fields rather than emitting key: undefined", () => {
    const config = parseStoreConfig({ apple: { info: { "en-US": { title: "Hello", subtitle: 42 } } } });
...
describe("android metadata folder round-trip (supply layout)", () => {
  it("writes supply .txt files and reads them back identically", () => {
    const android = {
      info: { "en-US": { title: "Hello", shortDescription: "short", fullDescription: "the full thing" } },
    };
    const dir = workDir();
