...
function review(overrides: Partial<CustomerReviewResource> & { id: string; rating: number }): CustomerReviewResource {
...
describe("listReviews", () => {
  it("throws an actionable error when the app record is missing", async () => {
    ...
    await expect(listReviews(api, "com.x.missing")).rejects.toThrow(/No App Store Connect app record/);
  });
  ...
  it("applies unansweredOnly client-side over the answered flag", async () => {
    const reviews = [review({ id: "r1", rating: 5, answered: true }), review({ id: "r2", rating: 4, answered: false })];
    const api = makeApi({ appId: { "com.x": "app1" }, reviews });
