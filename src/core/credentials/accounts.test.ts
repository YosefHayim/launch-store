...
import type { AccountRecord, AccountsFile } from '../types/index.js';
...
import { ACCOUNTS_FILE, CREDENTIALS_DIR } from '../services/paths.js';
import {
  addAccount,
  decideBuildAccount,
  decodeP8,
...
describe('registry mutations', () => {
  it('adds an account, stores its key namespaced, and makes it active', async () => {
    ...
    expect(listAccounts()[0]).toMatchObject({
      label: 'Personal',
      teamId: 'TEAM1',
  ...
  it('caches resolved identity in place', async () => {
    await addAccount({ keyId: 'AAAA1111', issuerId: 'issuer-a', label: 'Personal', p8: PEM });
    updateAccountIdentity('AAAA1111', 'TEAM9', ['Mapleleaf', 'SampleApp']);
    expect(listAccounts()[0]).toMatchObject({ teamId: 'TEAM9', apps: ['Mapleleaf', 'SampleApp'] });
  });
...
describe('migrateLegacyAccounts', () => {
  it("moves a legacy single key into the registry as the active 'default' account", async () => {
    ...
    expect(getActiveKeyId()).toBe('KEYXYZ7890');
    expect(listAccounts()[0]).toMatchObject({
      keyId: 'KEYXYZ7890',
      label: 'default',
    ...
    expect(await loadAscKeyById('KEYXYZ7890')).toEqual({
      keyId: 'KEYXYZ7890',
      issuerId: 'issuer-legacy',
      p8: PEM,
    });
