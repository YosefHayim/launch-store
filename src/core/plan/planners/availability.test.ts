...
describe("availabilityPlanner", () => {
  it("omits itself when no availability sidecar is present", async () => {
    const plan = await availabilityPlanner.plan(makeCtx(makeAscApiFake(), "/no/such/availability.config.json"));
    expect(plan.state).toBe("omitted");
  });

  it("skips with a creds hint when no Apple account is active", async () => {
    const plan = await availabilityPlanner.plan(makeCtx(null, writeConfig({ territories: ["USA"] })));
  ...
  it("reports the territory change a fresh config would apply (two-way)", async () => {
    const plan = await availabilityPlanner.plan(makeCtx(makeAscApiFake(), writeConfig({ territories: ["USA"] })));
    expect(plan.state).toBe("planned");
    if (plan.state !== "planned" || plan.scope !== "app") return;
    expect(plan.direction).toBe("two-way");
    expect(plan.apps[0]?.actions.some((a) => a.description.includes("store availability"))).toBe(true);
  });

  it("renders in sync when live availability already matches", async () => {
    const api = makeAscApiFake({
      getAppAvailability: vi
        .fn()
        .mockResolvedValue({ id: "avail1", availableInNewTerritories: false, availableTerritories: ["USA"] }),
    });
  ...
  it("captures a missing app record as a per-app error, not a thrown plan", async () => {
    ...
    const plan = await availabilityPlanner.plan(makeCtx(api, writeConfig({ territories: ["USA"] })));
    expect(plan.state).toBe("planned");
    if (plan.state !== "planned" || plan.scope !== "app") return;
