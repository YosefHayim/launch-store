...
function productsWith(subscriptions: SubscriptionConfig[]): Record<string, AppProducts> {
  return {
    "com.acme.alpha": {
      subscriptionGroups: [{ referenceName: "Pro", localizations: [{ locale: "en-US", name: "Pro" }], subscriptions }],
    },
  };
}
...
describe("playSubscriptionsPlanner", () => {
  it("omits itself when no app declares a Play-overridden subscription", async () => {
    const plan = await playSubscriptionsPlanner.plan(makeCtx(makePlayApi(), productsWith([APPLE_ONLY_SUB])));
  ...
  it("omits an app that has subscriptions but no Android package name", async () => {
    ...
    const plan = await playSubscriptionsPlanner.plan(makeCtx(makePlayApi(), productsWith([PLAY_SUB]), [noPackage]));
    expect(plan.state).toBe("omitted");
  });
