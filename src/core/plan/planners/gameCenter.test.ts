...
function makeCtx(api: AscSurfacesApi | null, gameCenter?: Record<string, GameCenterConfig>): PlanContext {
...
describe("gameCenterPlanner", () => {
  ...
  it("renders in sync when Game Center is already enabled and nothing is declared under it", async () => {
    const api = makeAscApiFake({ getGameCenterDetail: vi.fn().mockResolvedValue({ id: "detail1" }) });
