...
describe("accessibilityPlanner", () => {
  it("omits itself when no accessibility sidecar is present", async () => {
    const plan = await accessibilityPlanner.plan(makeCtx(makeAscApiFake(), "/no/such/accessibility.config.json"));
  ...
  it("reports an additive plan when a declaration doesn't exist yet", async () => {
    ...
    expect(plan.apps[0]?.actions.some((a) => a.description.includes("accessibility declaration"))).toBe(true);
  });

  it("renders in sync when the live declaration already matches", async () => {
    const api = makeAscApiFake({ listAccessibilityDeclarations: vi.fn().mockResolvedValue([PUBLISHED_IPHONE]) });
