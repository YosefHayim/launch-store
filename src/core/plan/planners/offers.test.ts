...
function makeCtx(api: AscSurfacesApi | null, products: Record<string, AppProducts> = {}): PlanContext {
...
describe("offersPlanner", () => {
  it("omits itself when no app declares offers or promoted purchases", async () => {
    const plan = await offersPlanner.plan(makeCtx(apiWithLiveSubscription(), { "com.acme.alpha": NO_OFFERS }));
  ...
  it("reports an additive plan to create a declared offer code that doesn't exist yet", async () => {
    const plan = await offersPlanner.plan(makeCtx(apiWithLiveSubscription(), { "com.acme.alpha": WITH_OFFER }));
    expect(plan.state).toBe("planned");
    if (plan.state !== "planned" || plan.scope !== "app") return;
    expect(plan.direction).toBe("additive");
    expect(
      plan.apps[0]?.actions.some((a) => a.description.includes('create offer code "Launch" on com.acme.pro')),
    ).toBe(true);
  });
