...
function makeApi(overrides: Partial<AscCatalogApi> = {}): AscCatalogApi {
  const base: AscCatalogApi = {
    ...
    findInAppPurchasePricePoint: vi.fn().mockResolvedValue({ id: "ipp", customerPrice: "0", territory: "USA" }),
...
function appDirWithListing(info: AppleLocaleInfo): string {
  ...
  tmpDirs.push(dir);
  writeFileSync(join(dir, "store.config.json"), JSON.stringify({ apple: { info: { "en-US": info } } }));
  return dir;
}
...
describe("listingPlanner", () => {
  ...
  it("reports the per-app diff a fresh listing would create", async () => {
    ...
    expect(
      plan.apps[0]?.actions.some((a) => a.description.startsWith("create listing [en-US] App Store version")),
    ).toBe(true);
  });
  ...
  it("is strictly read-only: never invokes a listing write endpoint", async () => {
    const api = makeApi();
    await listingPlanner.plan(makeCtx(api, appDirWithListing({ title: "Acme", description: "The best acme app." })));
