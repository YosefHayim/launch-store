...
import type { ReleaseAttributesConfig } from '@core/types/storeSurface.js';
...
const makeCtx = (
  ...
  if (releaseAttributes) config = { ...baseConfig, releaseAttributes };
  return {
    config,
    apps: [ALPHA],
...
describe('releaseConfigPlanner', () => {
  it('omits itself when no app declares release attributes', async () => {
    const plan = await runPlanner(releaseConfigPlanner, makeCtx(makeAscApiFake()));
    expect(plan.state).toBe('omitted');
  });
  it('skips with a creds hint when no Apple account is active', async () => {
    const plan = await runPlanner(releaseConfigPlanner, makeCtx(null, PRICED));
  ...
  it('reports the per-app price change a fresh config would apply (two-way)', async () => {
    const plan = await runPlanner(releaseConfigPlanner, makeCtx(makeAscApiFake(), PRICED));
  ...
  it('renders in sync when the live price already matches', async () => {
    ...
    const plan = await runPlanner(releaseConfigPlanner, makeCtx(api, PRICED));
    expect(plan.state).toBe('planned');
    if (plan.state !== 'planned') return;
    if (plan.scope !== 'app') return;
  ...
  it('is strictly read-only: never invokes a write endpoint', async () => {
    ...
    await runPlanner(releaseConfigPlanner, makeCtx(api, PRICED));
