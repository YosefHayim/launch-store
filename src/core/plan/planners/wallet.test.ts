...
const DECLARED: WalletConfig = { merchantIds: [{ identifier: "merchant.com.acme.app", name: "Acme Pay" }] };
...
describe("walletPlanner", () => {
  ...
  it("renders in sync when the merchant id already exists", async () => {
    const api = makeAscApiFake({
      listMerchantIds: vi.fn().mockResolvedValue([{ id: "m1", identifier: "merchant.com.acme.app" }]),
    });
