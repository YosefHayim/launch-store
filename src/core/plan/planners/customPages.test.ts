...
describe("customPagesPlanner", () => {
  it("omits itself when no custom-pages sidecar is present", async () => {
    const plan = await customPagesPlanner.plan(makeCtx(makeAscApiFake(), "/no/such/custom-pages.config.json"));
  ...
  it("reports the page a fresh config would create (two-way)", async () => {
    ...
    expect(plan.apps[0]?.actions.some((a) => a.description.includes("custom product page"))).toBe(true);
  });

  it("renders in sync when the live page already exists", async () => {
    const api = makeAscApiFake({ listCustomProductPages: vi.fn().mockResolvedValue([{ id: "pg1", name: "page1" }]) });
