...
describe("parsePackageManagerField", () => {
  it("splits a Corepack pin into manager + version", () => {
    ...
    expect(parsePackageManagerField("pnpm@9.1.0+sha512.abc")).toEqual({ name: "pnpm", version: "9.1.0" });
...
describe("detectPackageManager (filesystem)", () => {
  it("lets the packageManager field win over a lockfile", () => {
    ...
    expect(detectPackageManager(dir)).toMatchObject({ name: "yarn", source: "packageManager", corepackPinned: true });
  });
