...
export async function dispatch(
  ...
  if (violations.length > 0) {
    const detail = violations
      ...
      .join('; ');
    return {
      content: [{ type: 'text', text: `Invalid arguments for ${tool.name}: ${detail}` }],
      isError: true,
    };
  }
  try {
...
export async function startMcpServer(tools: readonly McpTool[] = ALL_TOOLS): Promise<void> {
  ...
  const server = new Server(
    { name: 'launch', version: serverVersion() },
    { capabilities: { tools: {} } },
  );
  ...
  process.stderr.write(
    `launch mcp: ${enabled.length} tool(s) exposed (${enabled.map((t) => t.name).join(', ')})\n`,
  );
