...
describe('formatAccountSummary', () => {
  ...
  it('degrades to label · team · key when no apps are cached', () => {
    expect(formatAccountSummary({ ...base, teamId: '5NS9ZUMYCS' })).toBe(
      'default · team 5NS9ZUMYCS · key F5763D97BY',
    );
  });
  ...
  it('lists up to three app names inline with no +N suffix', () => {
    expect(
      formatAccountSummary({
        ...base,
        teamId: '5NS9ZUMYCS',
        apps: ['OlyWell', 'Zaatar', 'Mealsy'],
      }),
    ).toBe('default · OlyWell, Zaatar, Mealsy · team 5NS9ZUMYCS · key F5763D97BY');
  });
  ...
  it('drops the leading label for the picker hint via includeLabel:false', () => {
    expect(
      formatAccountSummary(
        { ...base, teamId: '5NS9ZUMYCS', apps: ['OlyWell'] },
        { includeLabel: false },
      ),
    ).toBe('OlyWell · team 5NS9ZUMYCS · key F5763D97BY');
  });
});
...
describe('migrateLegacyAccounts', () => {
  it("moves a legacy single key into the registry as the active 'default' account", async () => {
    ...
    expect(await loadAscKeyById('QS5924Q3MD')).toEqual({
      keyId: 'QS5924Q3MD',
      issuerId: 'issuer-legacy',
      p8: PEM,
    });
