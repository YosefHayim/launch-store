...
describe("migrateLegacyAccounts", () => {
  it("moves a legacy single key into the registry as the active 'default' account", async () => {
    ...
    expect(listAccounts()[0]).toMatchObject({ keyId: "KEYXYZ7890", label: "default", issuerId: "issuer-legacy" });
    ...
    expect(await loadAscKeyById("KEYXYZ7890")).toEqual({ keyId: "KEYXYZ7890", issuerId: "issuer-legacy", p8: PEM });
  });
