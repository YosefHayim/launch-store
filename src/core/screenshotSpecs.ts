 ...
 * rest from them).
 */
export const DEFAULT_APPLE_DISPLAY_TYPES: readonly string[] = [
  'APP_IPHONE_67',
  'APP_IPAD_PRO_3GEN_129',
];
 ...
 * otherwise.
 */
export function validateAppleDimensions(
  displayType: string,
  width: number,
  height: number,
): DimensionVerdict {
  ...
  const matches = spec.accepted.some(
    ([w, h]) =>
      (width === w && height === h) || (spec.orientation === 'both' && width === h && height === w),
  );
  if (matches) return { ok: true };
  const expected = spec.accepted.map(fmt).join(', ');
  return {
    ok: false,
    reason: `${fmt([width, height])} is not a valid ${displayType} size (expected ${expected})`,
  };
}
...
export function validatePlayDimensions(width: number, height: number): DimensionVerdict {
  ...
  if (shorter < minSide || longer > maxSide) {
    return {
      ok: false,
      reason: `${fmt([width, height])} is outside Play's ${minSide}–${maxSide}px per-side range`,
    };
  }
 ...
 */
export function checkScreenshotFile(
  platform: Platform,
  target: string,
  path: string,
): FileDimensionCheck {
