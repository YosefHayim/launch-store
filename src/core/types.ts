...
export interface AppDescriptor {
  ...
  configPath: string;
  /** iOS bundle identifier (`ios.bundleIdentifier`), e.g. `com.example.sampleapp`. Undefined for Android-only apps. */
  bundleId?: string;
  /** Android application id (`android.package`), e.g. `com.example.sampleapp`. Undefined for iOS-only apps. */
  packageName?: string;
   ...
   * notification extensions, …), e.g. `["com.example.sampleapp.widget"]`. Each is provisioned exactly like
...
export interface AscKey {
  /** The key's ID (e.g. `KEYXYZ7890`). Globally unique per Apple, so it doubles as the account key. */
  keyId: string;
...
export interface AccountRecord {
  ...
  label: string;
  /** Apple Team ID (the bundle-id `seedId`, e.g. `ABCDE12345`), resolved from Apple. Absent until resolved. */
  teamId?: string;
...
export interface SigningAssets {
  /** Bundle identifier these assets sign, e.g. `com.example.sampleapp`. */
  bundleId: string;
  /** Apple Developer Team ID (e.g. `ABCDE12345`), read from the provisioning profile. */
  teamId: string;
