 ...
 */
export const summarizeRatings = (reviews: readonly ReviewDatum[]): RatingSummary => {
  ...
  let answeredRate = 0;
  if (total > 0) {
    average = round1(ratingSum / total);
 ...
 */
export const monthlyTrend = (reviews: readonly ReviewDatum[]): MonthlyRatingPoint[] => {
  ...
  for (const review of reviews) {
    ...
    if (bucket === undefined) bucket = { count: 0, sum: 0 };
    bucket.count += 1;
 ...
 */
export const buildAppInsights = (app: string, reviews: readonly ReviewDatum[]): AppInsights => {
 ...
 */
export const buildInsightsReport = (
  perApp: readonly {
    app: string;
    reviews: readonly ReviewDatum[];
  }[],
): InsightsReport => {
  return {
