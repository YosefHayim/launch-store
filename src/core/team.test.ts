...
function member(overrides: Partial<UserResource> = {}): UserResource {
  return { id: "u1", username: "jane@acme.com", firstName: "Jane", lastName: "Doe", roles: ["ADMIN"], ...overrides };
}

function invitation(overrides: Partial<UserInvitationResource> = {}): UserInvitationResource {
  return { id: "i1", email: "john@acme.com", firstName: "John", lastName: "Roe", roles: ["DEVELOPER"], ...overrides };
}
...
describe("inviteTeamMember", () => {
  ...
  it("passes through a provisioning override", async () => {
    ...
    expect(api.inviteUser).toHaveBeenCalledWith(expect.objectContaining({ provisioningAllowed: true }));
  });

  it("rejects an empty email", async () => {
    await expect(
      inviteTeamMember(makeApi(), { email: "  ", firstName: "Pat", lastName: "Lee", roles: ["DEVELOPER"] }),
    ).rejects.toThrow(/email is required/);
  });

  it("rejects when no roles are given", async () => {
    await expect(
      inviteTeamMember(makeApi(), { email: "new@acme.com", firstName: "Pat", lastName: "Lee", roles: ["  "] }),
    ).rejects.toThrow(/At least one role is required/);
  });

  it("rejects an unknown role with the valid list", async () => {
    await expect(
      inviteTeamMember(makeApi(), { email: "new@acme.com", firstName: "Pat", lastName: "Lee", roles: ["WIZARD"] }),
    ).rejects.toThrow(/Unknown role\(s\): WIZARD/);
  });

  it("rejects inviting someone who is already a member (case-insensitive)", async () => {
    const api = makeApi({ listUsers: vi.fn().mockResolvedValue([member({ username: "Jane@Acme.com" })]) });
    await expect(
      inviteTeamMember(api, { email: "jane@acme.com", firstName: "Jane", lastName: "Doe", roles: ["ADMIN"] }),
  ...
  it("rejects inviting someone who already has a pending invitation", async () => {
    const api = makeApi({ listUserInvitations: vi.fn().mockResolvedValue([invitation({ email: "john@acme.com" })]) });
    await expect(
      inviteTeamMember(api, { email: "john@acme.com", firstName: "John", lastName: "Roe", roles: ["DEVELOPER"] }),
...
describe("removeTeamMember", () => {
  it("deletes an accepted member matched by email (case-insensitive)", async () => {
    const api = makeApi({ listUsers: vi.fn().mockResolvedValue([member({ id: "u9", username: "jane@acme.com" })]) });
  ...
  it("cancels a pending invitation when no member matches", async () => {
    const api = makeApi({
      listUserInvitations: vi.fn().mockResolvedValue([invitation({ id: "i9", email: "john@acme.com" })]),
    });
    const outcome = await removeTeamMember(api, "john@acme.com");
    expect(outcome).toEqual({ kind: "invitation", invitation: expect.objectContaining({ id: "i9" }) });
