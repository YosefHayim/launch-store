...
describe("inspectDoctor", () => {
  ...
  it("reports the Android SDK as ok when ANDROID_HOME is set", async () => {
    const report = await inspectDoctor(context({ platform: "android", androidSdk: "/opt/android-sdk" }));
    expect(report.checks.some((c) => c.status === "ok" && c.title.includes("/opt/android-sdk"))).toBe(true);
  });
  ...
  it("isolates a throwing section as a single fail without sinking the rest", async () => {
    ...
    expect(failed).toEqual([{ status: "fail", title: "Credentials check failed", detail: "keychain locked" }]);
    expect(report.ok).toBe(false);
    expect(report.checks.some((c) => c.title.startsWith("Package manager:"))).toBe(true);
  });
