...
describe("probeKeyPermissions", () => {
  ...
  it("flags a 403 as forbidden for just that feature, leaving the rest available", async () => {
    const api = makeApi({ listCustomerReviews: vi.fn().mockRejectedValue(new AscRequestError("Forbidden", 403)) });
  ...
  it("maps a 401 to unauthorized", async () => {
    const api = makeApi({ listBetaGroups: vi.fn().mockRejectedValue(new AscRequestError("Unauthorized", 401)) });
    const results = await probeKeyPermissions(api, "app1");
  ...
  it("maps a non-HTTP failure to inconclusive, preserving the error message", async () => {
    const api = makeApi({ listAppStoreVersions: vi.fn().mockRejectedValue(new Error("network down")) });
...
describe("formatPermissionLine", () => {
  ...
  it("renders unauthorized", () => {
    expect(formatPermissionLine({ ...base, status: "unauthorized" })).toContain("unauthorized (401)");
  });

  it("renders inconclusive with the detail", () => {
    expect(formatPermissionLine({ ...base, status: "inconclusive", detail: "no app record to probe" })).toBe(
      "• Feature X — couldn't determine (no app record to probe)",
    );
  });

  it("renders inconclusive without a detail", () => {
    expect(formatPermissionLine({ ...base, status: "inconclusive" })).toBe("• Feature X — couldn't determine");
  });
});
