 ...
 * to name them in order: `src/core/build/pipeline.ts` (the live flow, see its `log.step` calls) and
 * `src/core/terminal/tour.ts` (the first-run walkthrough that narrates each phase). Keeping the list
 * here means the tour can't silently fall out of step with the pipeline — a test asserts the tour covers
 * exactly these phases, so adding a phase to the flow forces it into the tour too.
 *
