...
export const autoReleaseAt = (allocatedAtIso: string): string => {
...
export const isReleasable = (allocatedAtIso: string, now: number = Date.now()): boolean => {
 ...
 */
export const consentMessage = (): string => {
  return [
...
export const costBanner = (
  ...
  let hostId = 'remote host';
  if (handle.instanceId !== undefined) hostId = handle.instanceId;
  ...
  return `host ${hostId} up ${formatAge(ageMs)}, ~${usd(costForDurationUsd(ageMs))} so far, releasable after ${releaseLabel}`;
};
