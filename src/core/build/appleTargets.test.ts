...
describe('discoverExtensionBundleIds - locate and read the generated project', () => {
  ...
  it('discovers the widget extension from a generated ios/ project', async () => {
    ...
    try {
      ...
      await expect(
        Effect.runPromise(
          discoverExtensionBundleIds(nativeDir, 'com.example.sampleapp').pipe(
  ...
  it('returns [] for a single-target project (no extensions to provision)', async () => {
    const root = mkdtempSync(join(tmpdir(), 'launch-targets-'));
    try {
      const nativeDir = join(root, 'ios');
  ...
  it('returns [] when the native project has not been generated yet', async () => {
    const root = mkdtempSync(join(tmpdir(), 'launch-targets-'));
    try {
      await expect(
        Effect.runPromise(
          discoverExtensionBundleIds(join(root, 'ios'), 'com.example.sampleapp').pipe(
            Effect.provide(NodeContext.layer),
          ),
        ),
      ).resolves.toEqual([]);
...
describe('findPbxproj - locate the project file in a native dir', () => {
  it('finds the .xcodeproj/project.pbxproj and returns null when absent', async () => {
    const root = mkdtempSync(join(tmpdir(), 'launch-targets-'));
    try {
      await expect(
        Effect.runPromise(findPbxproj(join(root, 'ios')).pipe(Effect.provide(NodeContext.layer))),
      ).resolves.toBeNull();
      ...
      await expect(
        Effect.runPromise(findPbxproj(join(root, 'ios')).pipe(Effect.provide(NodeContext.layer))),
      ).resolves.toBe(join(projectDir, 'project.pbxproj'));
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
describe('writeManualSigningToProject - stamp the generated project on disk', () => {
  it('rewrites the pbxproj and reports the change, then no-ops on a second run', async () => {
    const root = mkdtempSync(join(tmpdir(), 'launch-targets-'));
    try {
      const nativeDir = join(root, 'ios');
      ...
      await expect(
        Effect.runPromise(
          writeManualSigningToProject(nativeDir, SIGNING).pipe(Effect.provide(NodeContext.layer)),
        ),
      ).resolves.toBe(true);
  ...
  it('returns false when the native project has not been generated yet', async () => {
    const root = mkdtempSync(join(tmpdir(), 'launch-targets-'));
    try {
      await expect(
        Effect.runPromise(
          writeManualSigningToProject(join(root, 'ios'), SIGNING).pipe(
            Effect.provide(NodeContext.layer),
          ),
        ),
      ).resolves.toBe(false);
