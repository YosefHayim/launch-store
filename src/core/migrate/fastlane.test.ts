...
describe('migrateFastlane', () => {
  ...
  it('reports a custom lane with no recognized actions as a manual follow-up', () => {
    ...
    const manual = notesAt(migrateFastlane(dir, [app()]).notes, 'manual').map(
      (note) => note.message,
    );
  ...
  it('maps gym/pilot/deliver/supply to Launch commands', () => {
    const mapped = notesAt(migrateFastlane(dir, [app()]).notes, 'mapped').map(
      (note) => note.message,
    );
  ...
  it('reports the Appfile app_identifier and detected ids as info', () => {
    const info = notesAt(
      migrateFastlane(dir, [app({ packageName: 'com.acme.alpha' })]).notes,
      'info',
  ...
  it('skips store.config.json when one already exists', () => {
    ...
    expect(
      notesAt(result.notes, 'skipped').some((note) => note.message.includes('store.config.json')),
    ).toBe(true);
  });
  ...
  it('imports the fastlane/metadata folder into store.config.json and drops the Deliverfile follow-up', () => {
    ...
    expect(
      notesAt(result.notes, 'mapped').some((note) =>
        note.message.includes('Imported your fastlane metadata'),
      ),
    ).toBe(true);
    expect(
      notesAt(result.notes, 'manual').some((note) => note.message.includes('Deliverfile')),
    ).toBe(false);
  });
