...
const BUILD: BuildResource = { id: "build-1", version: "42", processingState: "VALID", expired: false };
...
describe("parseBetaReviewConfig", () => {
  ...
  it("rejects a non-object, a missing/empty map, and a non-string note", () => {
    ...
    expect(() => parseBetaReviewConfig({ whatToTest: { "en-US": "" } })).toThrow(/must be a non-empty string/);
  });
});
...
describe("reconcileBetaReview", () => {
  it("sets a new What-to-Test note and submits for review (apply)", async () => {
    ...
    expect(calls.created).toEqual([{ buildId: "build-1", locale: "en-US", whatsNew: "Bug fixes." }]);
  ...
  it("updates an existing note when the text differs, and leaves an identical one alone", async () => {
    ...
    await reconcileBetaReview(api, { appId: "app-1", whatToTest: WHATS_NEW, submitForReview: false, dryRun: false });
  ...
  it("targets a specific build by version, and rejects an expired or missing one", async () => {
    ...
    expect(calls.created).toEqual([{ buildId: "build-2", locale: "en-US", whatsNew: "Bug fixes." }]);
  ...
  it("throws when there's no eligible build", async () => {
    const { api } = makeApi({ builds: [{ id: "p", version: "1", processingState: "PROCESSING", expired: false }] });
    await expect(
      reconcileBetaReview(api, { appId: "app-1", whatToTest: {}, submitForReview: true, dryRun: false }),
  ...
  it("captures a failed submission without aborting the notes that already applied", async () => {
    const { api } = makeApi({});
    api.createBetaAppReviewSubmission = () => Promise.reject(new Error("build is still processing"));
    const report = await reconcileBetaReview(api, {
      appId: "app-1",
      whatToTest: WHATS_NEW,
      submitForReview: true,
    ...
    expect(report.actions.find((action) => action.status === "failed")?.error).toBe("build is still processing");
  });
});
