...
import { downloadFeedbackAttachments, listBetaFeedback, type AscFeedbackApi } from "./testflightFeedback.js";
...
function makeApi(opts: {
  ...
  return {
    ...
    findBuildByVersion: vi.fn((_appId: string, version: number) => Promise.resolve(opts.builds?.[version] ?? null)),
...
describe("listBetaFeedback", () => {
  it("throws an actionable error when the app record is missing", async () => {
    ...
    await expect(listBetaFeedback(api, "com.x.missing")).rejects.toThrow(/No App Store Connect app record/);
  });
  ...
  it("--build resolves the version to a build id and pushes it as a server filter", async () => {
    ...
    expect(api.listBetaFeedbackCrashSubmissions).toHaveBeenCalledWith("app1", { buildId: "build-42" });
  });
  ...
  it("--build rejects a non-numeric CFBundleVersion before any network call", async () => {
    ...
    await expect(listBetaFeedback(api, "com.x", { build: "1.2.0" })).rejects.toThrow(/CFBundleVersion/);
  ...
  it("omits an empty screenshots array on a screenshot submission with no usable images", async () => {
    const api = makeApi({ appId: { "com.x": "app1" }, screenshots: [{ id: "s1", screenshots: [] }] });
...
describe("downloadFeedbackAttachments", () => {
  it("writes one file per screenshot, named <id>-<n>.png, and skips crashes", async () => {
    ...
    const feedback: BetaFeedback[] = [
      { id: "c1", kind: "crash" },
      { id: "s1", kind: "screenshot", screenshots: [{ url: "https://a/1.png" }, { url: "https://a/2.png" }] },
    ];
    const outDir = mkdtempSync(join(tmpdir(), "launch-feedback-"));
    const written = await downloadFeedbackAttachments(api, feedback, outDir);

    expect(written.map((entry) => entry.path)).toEqual([join(outDir, "s1-1.png"), join(outDir, "s1-2.png")]);
