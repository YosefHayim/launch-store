 ...
 *
 * Validation enters through `config/schema.ts` (ADR 0013). The committed JSON Schema is loaded from
 * disk for print/render paths (`launch config schema`, MCP `config_schema`); it is regenerated by
 * `pnpm docs:gen` from `LaunchConfigEffectSchema`.
 */
