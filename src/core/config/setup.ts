...
import type { AppDescriptor, Platform } from '../types/index.js';
...
import { hostOsLabel, hostResources, isMac } from '../services/os.js';
...
export function formatSetupBoard(readiness: SetupReadiness): string[] {
  ...
  for (const group of readiness.groups) {
    ...
    for (const check of group.rows) {
      lines.push(
        `  ${mark(check.status)} ${check.label}${check.detail ? ` — ${check.detail}` : ''}`,
      );
    }
  }
  return lines;
...
export function toolchainReadinessRows(tools: Tool[], present: Set<string>): ReadinessRow[] {
  return tools.map((tool) => {
    ...
    if (tool.tier === 'recommended')
      return row(tool.label, 'info', `recommended — ${fixHint(tool)}`);
...
function configRows(apps: AppDescriptor[]): ReadinessRow[] {
  ...
  return [
    hasConfig
      ? row('launch.config.ts', 'ok', 'present')
      : row('launch.config.ts', 'todo', 'run: launch init'),
    apps.length > 0
...
async function appleAccountRows(apps: AppDescriptor[]): Promise<ReadinessRow[]> {
  ...
  if (!account) {
    return [
      row('Apple account', 'todo', 'import your App Store Connect key: launch creds set-key'),
    ];
  }
  const rows = [
    row(
      `Apple account: ${account.label}`,
      'ok',
      formatAccountSummary(account, { includeLabel: false }),
    ),
  ];
...
async function playAccountRows(apps: AppDescriptor[]): Promise<ReadinessRow[]> {
  ...
  if (!json) {
    return [
      row('Play service account', 'todo', 'import it: launch creds set-key --platform android'),
    ];
  }
...
async function appConfigRows(apps: AppDescriptor[], platform: Platform): Promise<ReadinessRow[]> {
  ...
  for (const app of apps) {
    ...
    for (const finding of findings) {
      rows.push(
        row(app.name, finding.severity === 'error' ? 'todo' : 'info', formatFinding(finding)),
      );
    }
  }
  return rows;
...
export async function runSetup(options: SetupOptions): Promise<void> {
  ...
  log.notice(
    'Launch setup',
    `Getting your ${platformLabel(platform)} app ready to ship — hands-off where it's safe.`,
  );
  ...
  if (
    ...
    isMac() &&
    (isInteractive() || yes) &&
    (await missingRequiredTools()).length > 0
  ) {
  ...
  if (!isInteractive())
    log.info("Re-run `launch setup` once those are done to confirm everything's green.");
}
