...
const runEnvEffect = <Success, Failure>(
  envEffect: Effect.Effect<Success, Failure, NodeContext.NodeContext>,
): Promise<Success> => Effect.runPromise(envEffect.pipe(Effect.provide(NodeContext.layer)));
...
describe('loadDotenvFile', () => {
  it('returns an empty object when the file is absent', async () => {
    expect(await runEnvEffect(loadDotenvFile(join(makeTempDir(), 'nope.env')))).toEqual({});
  });
  it('parses an existing file', async () => {
    const dir = makeTempDir();
    const path = join(dir, '.env');
    writeFileSync(path, 'API_URL=https://example.com\n');
    expect(await runEnvEffect(loadDotenvFile(path))).toEqual({ API_URL: 'https://example.com' });
  });
});
describe('missingKeys - fail before a wasted build', () => {
  it('returns nothing when there is no .env.example to validate against', async () => {
    expect(await runEnvEffect(missingKeys(makeTempDir(), { ANYTHING: '1' }))).toEqual([]);
  });
  it('flags keys the example documents but the env omits or leaves empty', async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, '.env.example'), 'API_URL=\nFEATURE_FLAG=\nPRESENT=\n');
    expect(
      (await runEnvEffect(missingKeys(dir, { PRESENT: 'yes', EMPTY_ONE: '' }))).sort(),
    ).toEqual(['API_URL', 'FEATURE_FLAG']);
  });
  it("exempts an excluded key so documenting a backend-only secret doesn't trip the gate", async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, '.env.example'), 'API_URL=\nOPENAI_API_KEY=\n');
    // OPENAI_API_KEY is documented but excluded -> not "missing"; API_URL still is.
    expect(await runEnvEffect(missingKeys(dir, {}, ['OPENAI_API_KEY']))).toEqual(['API_URL']);
  });
  it('exempts keys matched by an envExclude prefix even when no layer sets them', async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, '.env.example'), 'API_URL=\nOPENAI_API_KEY=\nOPENAI_ORG_ID=\n');
    // `OPENAI_*` covers both OPENAI_ keys though neither is set; API_URL is still missing.
    expect(await runEnvEffect(missingKeys(dir, {}, ['OPENAI_*']))).toEqual(['API_URL']);
  });
});
...
describe('resolveEnv - the one precedence ladder', () => {
  it('layers lowest->highest: .env < .env.<profile> < .env.local < profile env: < secrets < flags', async () => {
    const dir = makeTempDir();
    ...
    const resolved = await runEnvEffect(
      resolveEnv({
        appDir: dir,
  ...
  it('omits .env.local unless includeLocal is set', async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, '.env'), 'X=base\n');
    writeFileSync(join(dir, '.env.local'), 'X=local\n');
    expect(
      (await runEnvEffect(resolveEnv({ appDir: dir, profileName: 'production' }))).values,
    ).toEqual({
      X: 'base',
    });
    expect(
      (
        await runEnvEffect(
          resolveEnv({ appDir: dir, profileName: 'production', includeLocal: true }),
        )
      ).values,
    ).toEqual({ X: 'local' });
  });
  it('renames the base file via envFile and still derives .env.<profile> by name', async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, 'custom.env'), 'A=fromCustom\nB=fromCustom\n');
    writeFileSync(join(dir, '.env.staging'), 'B=fromProfileFile\n');
    const resolved = await runEnvEffect(
      resolveEnv({ appDir: dir, profileName: 'staging', envFile: 'custom.env' }),
    );
    expect(resolved.values).toEqual({ A: 'fromCustom', B: 'fromProfileFile' });
    expect(resolved.sources).toEqual({ A: 'custom.env', B: '.env.staging' });
  });
  it('returns empty maps when nothing resolves', async () => {
    const resolved = await runEnvEffect(
      resolveEnv({ appDir: makeTempDir(), profileName: 'production' }),
    );
    expect(resolved.values).toEqual({});
    expect(resolved.sources).toEqual({});
    expect(resolved.excluded).toEqual([]);
  });
  it('drops envExclude names after every layer merges - even an explicit --env - and records them', async () => {
    const dir = makeTempDir();
  ...
  it('records no exclusions when envExclude is absent or empty', async () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, '.env'), 'A=1\n');
    expect(
      (await runEnvEffect(resolveEnv({ appDir: dir, profileName: 'production' }))).excluded,
    ).toEqual([]);
    expect(
      (await runEnvEffect(resolveEnv({ appDir: dir, profileName: 'production', envExclude: [] })))
        .excluded,
    ).toEqual([]);
  });
  it('supports PREFIX* entries so a family of backend keys collapses to one line', async () => {
    const dir = makeTempDir();
...
describe('parseCliEnv - repeated --env KEY=VAL', () => {
  it('splits on the first = and keeps = in the value', () => {
    expect(Effect.runSync(parseCliEnv(['A=1', 'DSN=postgres://u:p@h/db?x=1']))).toEqual({
      A: '1',
      DSN: 'postgres://u:p@h/db?x=1',
    });
  });
  it('throws on a pair with no = or an empty key', () => {
    expect(() => Effect.runSync(parseCliEnv(['NOPE']))).toThrow(/Invalid --env/);
    expect(() => Effect.runSync(parseCliEnv(['=value']))).toThrow(/empty/);
  });
});
describe('formatEnvTable - masked provenance for --print-env', () => {
  ...
  it('reports when nothing resolved', () => {
    expect(formatEnvTable({ values: {}, sources: {}, excluded: [] })).toBe(
      '(no env vars resolved)',
    );
  });
});
