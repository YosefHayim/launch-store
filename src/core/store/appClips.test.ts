...
import type { AppClipsConfig } from '../types/index.js';
...
function makeApi(state: Partial<State>): { api: AscAppClipsApi; calls: Calls } {
  ...
  const api: AscAppClipsApi = {
    ...
    findEditableAppStoreVersion: () =>
      Promise.resolve(full.editableVersionId ? { id: full.editableVersionId } : null),
    listAppClips: () => Promise.resolve(full.clips),
    listAppClipDefaultExperiences: (appClipId) =>
      Promise.resolve(full.experiences[appClipId] ?? []),
...
describe('parseAppClipsConfig', () => {
  ...
  it('rejects an invalid action and a non-string subtitle', () => {
    expect(() => parseAppClipsConfig({ clips: { c: { action: 'TAP' } } })).toThrow(
      /OPEN \/ VIEW \/ PLAY/,
    );
    expect(() =>
      parseAppClipsConfig({ clips: { c: { localizations: { 'en-US': { subtitle: 1 } } } } }),
...
describe('reconcileAppClips', () => {
  ...
  it('throws when the app has no App Store Connect record', async () => {
    ...
    await expect(
      reconcileAppClips(api, { bundleId: 'com.acme.app', config: ONE_CLIP, dryRun: true }),
  ...
  it("skips every clip when there's no editable App Store version", async () => {
    ...
    const report = await reconcileAppClips(api, {
