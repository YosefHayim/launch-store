...
import { historyIndexKey, historySnapshotKey, manifestKey, manifestSignatureKey } from "./otaManifest.js";
...
describe("publishOtaPlatform", () => {
  ...
  it("uploads bundle, manifest, snapshot, and history; signs when a signer is given", async () => {
    ...
    expect(keys).toContain(historySnapshotKey("production", "ios", "1.0.0", result.manifestId ?? ""));

    const history = JSON.parse(storage.objects.get(historyIndexKey("production", "ios")) ?? "[]") as {
      signed: boolean;
    }[];
  ...
  it("publishes unsigned (no signature object) when signer is null", async () => {
    ...
    await publishOtaPlatform(
      { storage, distDir, metadata, platform: "ios", channel: "production", runtimeVersion: "1.0.0", signer: null },
      log,
    );
    expect([...storage.objects.keys()]).not.toContain(manifestSignatureKey("production", "ios", "1.0.0"));
  });
