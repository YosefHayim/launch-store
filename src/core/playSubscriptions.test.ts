...
function makeApi(
  ...
  options: { reachable?: boolean; offersByBasePlan?: Record<string, SubscriptionOfferResource[]> } = {},
): { api: PlaySubscriptionsApi; calls: Calls } {
  const calls: Calls = { created: [], patched: [], activatedBasePlans: [], createdOffers: [], activatedOffers: [] };
...
describe("buildOffer", () => {
  it("builds a free-trial offer covering the base plan's regions", () => {
    const offer = buildOffer("com.acme.pro", "p1m", ["US", "GB"], { offerId: "trial", freeTrialDuration: "P1W" });
  ...
  it("builds an introductory-price offer over the priced regions", () => {
    ...
    expect(offer.phases).toEqual([
      {
        recurrenceCount: 3,
        regionalConfigs: [{ regionCode: "US", price: { currencyCode: "USD", units: "4", nanos: 990000000 } }],
      },
    ]);
  });
  ...
  it("rejects an offer that discounts nothing and one whose phases share no region", () => {
    expect(() => buildOffer("p", "bp", ["US"], { offerId: "empty" })).toThrow(/neither a free trial nor intro/);
    expect(() =>
      buildOffer("p", "bp", ["GB"], {
        offerId: "split",
...
describe("reconcilePlaySubscriptions", () => {
  it("throws when the Play app record is unreachable", async () => {
    ...
    await expect(
      reconcilePlaySubscriptions(api, { packageName: "com.acme.app", subscriptions: [sub()], dryRun: true }),
  ...
  it("creates a subscription + base plan, activates the plan, then creates + activates its offers", async () => {
    ...
    expect(created.listings).toEqual([{ languageCode: "en-US", title: "Pro Monthly", description: "All features" }]);
    expect(calls.activatedBasePlans).toEqual([{ productId: "com.acme.pro.monthly", basePlanId: "p1m" }]);
    expect(calls.createdOffers[0]?.offerId).toBe("trial");
    expect(calls.activatedOffers).toEqual([{ productId: "com.acme.pro.monthly", basePlanId: "p1m", offerId: "trial" }]);
  ...
  it("patches listings (mask 'listings') when a title drifts, preserving locales it doesn't manage", async () => {
    ...
    const { api, calls } = makeApi([existing]);
    await reconcilePlaySubscriptions(api, { packageName: "com.acme.app", subscriptions: [sub()], dryRun: false });
    expect(calls.patched).toHaveLength(1);
    expect(calls.patched[0]?.updateMask).toBe("listings");
    const languages = (calls.patched[0]?.subscription.listings ?? []).map((listing) => listing.languageCode).sort();
  ...
  it("adds + activates a base plan missing from an existing subscription, re-sending live plans without their state", async () => {
    const existing: SubscriptionResource = {
      productId: "com.acme.pro.monthly",
      listings: [{ languageCode: "en-US", title: "Pro Monthly", description: "All features" }],
      basePlans: [{ basePlanId: "p1y", state: "ACTIVE", autoRenewingBasePlanType: { billingPeriodDuration: "P1Y" } }],
    };
    const { api, calls } = makeApi([existing]);
    await reconcilePlaySubscriptions(api, { packageName: "com.acme.app", subscriptions: [sub()], dryRun: false });
  ...
  it("creates a missing offer but skips one that already exists", async () => {
    ...
    const { api, calls } = makeApi([existing], {
      offersByBasePlan: { "com.acme.pro.monthly/p1m": [{ offerId: "trial", phases: [], regionalConfigs: [] }] },
    });
        ...
        sub({
          play: {
            ...
            offers: [
              ...
              { offerId: "intro", introPrices: { US: { priceMicros: "4990000", currency: "USD" } } },
            ],
          },
        }),
      ],
  ...
  it("records a failed action for an offer config that discounts nothing, without aborting", async () => {
    ...
    const result = await reconcilePlaySubscriptions(api, {
      packageName: "com.acme.app",
      subscriptions: [
        sub({ play: { prices: { US: { priceMicros: "9990000", currency: "USD" } }, offers: [{ offerId: "broken" }] } }),
      ],
      dryRun: false,
    });
