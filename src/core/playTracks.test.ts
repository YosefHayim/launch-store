...
describe("buildRelease", () => {
  ...
  it("requires a fraction for inProgress and forbids one for completed/draft", () => {
    expect(() => buildRelease({ versionCodes: ["12"], status: "inProgress" })).toThrow(/needs a rollout fraction/);
    expect(() => buildRelease({ versionCodes: ["12"], status: "completed", userFraction: 0.5 })).toThrow(
      /can't carry a rollout fraction/,
    );
  });

  it("rejects an empty version list and an out-of-range fraction", () => {
    expect(() => buildRelease({ versionCodes: [], status: "completed" })).toThrow(/at least one version code/);
    expect(() => buildRelease({ versionCodes: ["12"], status: "inProgress", userFraction: 1 })).toThrow(
      /between 0 and 1/,
    );
  });
});
