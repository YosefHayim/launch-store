...
describe("diffSnapshots", () => {
  it("reports an entity present only in the newer snapshot as added", () => {
    ...
    expect(diff.entries[0]).toMatchObject({ change: "added", key: "coins", app: "alpha", store: "appstore" });
  });
  ...
  it("reports a same-key entity with differing data as changed", () => {
    const diff = diffSnapshots(snap([entity("coins", { price: 1 })]), snap([entity("coins", { price: 2 })]));
  ...
  it("treats reordered keys as no change", () => {
    const diff = diffSnapshots(snap([entity("coins", { a: 1, b: 2 })]), snap([entity("coins", { b: 2, a: 1 })]));
    ...
    const before: Snapshot = {
      ...
      reports: [
        ...snap([entity("dup", { id: 1 })]).reports,
        {
          ...
          outcome: {
            state: "captured",
            apps: [{ app: "alpha", identifier: "com.acme.alpha", entities: [entity("dup", { id: 1 })] }],
          },
        },
      ],
    };
