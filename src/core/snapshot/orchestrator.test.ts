...
function source(id: string, store: SnapshotStore, capture: () => Promise<SourceCapture>): SnapshotSource {
...
function captured(count: number): SourceCapture {
  const entities = Array.from({ length: count }, (_, i) => ({ key: `p${i}`, summary: `p${i}`, data: { id: i } }));
...
describe("captureSnapshot", () => {
  ...
  it("records a skipped surface without failing the run", async () => {
    const result = await captureSnapshot(
      makeCtx(),
      [source("a", "play", () => Promise.resolve({ state: "skipped", reason: "no Play credentials" }))],
      META,
    );
