...
describe("checkConfigSemantics", () => {
  ...
  describe("release", () => {
    it("requires earliestReleaseDate for a SCHEDULED release", () => {
      expect(paths({ release: { releaseType: "SCHEDULED" } })).toEqual(["release.earliestReleaseDate"]);
    });

    it("accepts a SCHEDULED release with a valid instant", () => {
      expect(paths({ release: { releaseType: "SCHEDULED", earliestReleaseDate: "2026-01-31T09:00:00Z" } })).toEqual([]);
    });

    it("rejects an unparseable earliestReleaseDate", () => {
      expect(paths({ release: { earliestReleaseDate: "next tuesday" } })).toEqual(["release.earliestReleaseDate"]);
    });

    it("flags per-locale release notes missing the primary locale", () => {
      expect(paths({ release: { releaseNotes: { "fr-FR": "Corrections." } } })).toEqual(["release.releaseNotes"]);
    });
    ...
    it("honors an explicit primaryLocale when checking per-locale notes", () => {
      expect(paths({ release: { primaryLocale: "fr-FR", releaseNotes: { "fr-FR": "Corrections." } } })).toEqual([]);
      expect(paths({ release: { primaryLocale: "fr-FR", releaseNotes: { "en-US": "Bug fixes." } } })).toEqual([
  ...
  it("collects multiple findings in a stable order (storage, release, profiles, retention)", () => {
    ...
    ).toEqual(["storageConfig", "release.earliestReleaseDate", "profiles.production.rollout", "artifactRetentionDays"]);
  });
});
