 ...
 */
export async function resolveSecretRef(value: string, label = "secret", secretStore?: SecretStore): Promise<string> {
  if (value.startsWith(ENV_PREFIX)) {
    const name = value.slice(ENV_PREFIX.length);
    if (name === "") throw new Error(`${label}: an \`env:\` reference needs a variable name (e.g. \`env:DEMO_PW\`).`);
    const resolved = process.env[name];
    if (resolved === undefined || resolved === "") {
      throw new Error(`${label} references \`env:${name}\`, but the environment variable ${name} is not set.`);
    }
    return resolved;
  }
  if (value.startsWith(KEYCHAIN_PREFIX)) {
    ...
    if (account === "") {
      throw new Error(`${label}: a \`keychain:\` reference needs an account name (e.g. \`keychain:my-app-review\`).`);
    }
