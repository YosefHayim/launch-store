...
class FakeOffersApi implements AscOffersApi {
  ...
  findSubscriptionPricePoint(
    ...
    return Promise.resolve({ id: `pp-${territory}-${customerPrice}`, customerPrice: String(customerPrice), territory });
  }
  ...
  createPromotionalOffer(input: PromotionalOfferCreate): Promise<PromotionalOfferResource> {
    ...
    return Promise.resolve({ id: `po-${input.offerCode}`, name: input.name, offerCode: input.offerCode });
  }
...
function productsWith(overrides: Partial<SubscriptionConfig>): AppProducts {
  ...
  return {
    subscriptionGroups: [
      { referenceName: "Main", localizations: [{ locale: "en-US", name: "Main" }], subscriptions: [subscription] },
    ],
  };
}
...
describe("reconcileOffers — offer codes", () => {
  it("creates a missing offer code with its prices resolved to price points", async () => {
    ...
    expect(api.createdOfferCodes[0]?.prices).toEqual([{ territory: "USA", pricePointId: "pp-USA-4.99" }]);
  });
...
describe("reconcileOffers — promoted purchases", () => {
  it("promotes a missing product and reorders to the declared order", async () => {
    const api = new FakeOffersApi();
    api.iaps = [{ id: "iap-1", productId: "com.acme.coins", name: "Coins", inAppPurchaseType: "CONSUMABLE" }];
    api.promoted = [
      { id: "pp-existing", inAppPurchaseId: "iap-1", subscriptionId: null, enabled: true, visibleForAllUsers: true },
    ];
    const products: AppProducts = {
      ...productsWith({}),
      promotedPurchases: [{ productId: "com.acme.pro" }, { productId: "com.acme.coins" }],
    };
    const report = await reconcileOffers(api, { bundleId: "com.acme.app", dryRun: false, products });
    ...
    expect(report.actions.some((action) => action.description.startsWith("reorder promoted purchases"))).toBe(true);
  });

  it("skips a promoted product that maps to no subscription or IAP", async () => {
    const api = new FakeOffersApi();
    const products: AppProducts = { ...productsWith({}), promotedPurchases: [{ productId: "com.acme.ghost" }] };
    const report = await reconcileOffers(api, { bundleId: "com.acme.app", dryRun: false, products });
    expect(report.actions[0]?.status).toBe("skipped");
