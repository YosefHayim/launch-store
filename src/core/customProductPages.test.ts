...
describe("parseCustomProductPagesConfig", () => {
  ...
  it("rejects a non-object, an empty list, a duplicate name, and bad promo text", () => {
    ...
    expect(() => parseCustomProductPagesConfig({ pages: [{ name: "A", promotionalText: { "en-US": "" } }] })).toThrow(
...
describe("reconcileCustomProductPages", () => {
  ...
  it("creates a missing page and sets its promotional text (apply)", async () => {
    ...
    const report = await reconcileCustomProductPages(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: false });
    expect(calls.createdPages).toEqual(["Spring Sale"]);
    expect(calls.createdLocs).toEqual([{ versionId: "ver-1", locale: "en-US", promotionalText: "50% off this week!" }]);
  ...
  it("updates promotional text on an existing page when it differs, and skips an identical one", async () => {
    ...
    await reconcileCustomProductPages(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: false });
    ...
    const report = await reconcileCustomProductPages(api2, { bundleId: "com.acme.app", config: CONFIG, dryRun: false });
  ...
  it("plans but performs nothing on a dry-run (new page)", async () => {
    const { api, calls } = makeApi({});
    const report = await reconcileCustomProductPages(api, { bundleId: "com.acme.app", config: CONFIG, dryRun: true });
  ...
  it("skips a page's promotional text when the page create failed", async () => {
    ...
    expect(report.actions.find((action) => action.status === "failed")?.error).toBe("page name taken");
  });
});
