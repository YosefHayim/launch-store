...
import { type AscAvailabilityApi, parseAvailabilityConfig, reconcileAvailability } from "./availability.js";
...
describe("parseAvailabilityConfig", () => {
  it("parses and uppercases territory codes, with the optional flag", () => {
    expect(parseAvailabilityConfig({ availableInNewTerritories: true, territories: ["usa", "gbr"] })).toEqual({
  ...
  it("rejects a non-object, a missing/empty list, a non-string code, and a non-boolean flag", () => {
    ...
    expect(() => parseAvailabilityConfig({ territories: ["USA", ""] })).toThrow(/territories\[1\] must be a non-empty/);
    expect(() => parseAvailabilityConfig({ territories: ["USA"], availableInNewTerritories: "yes" })).toThrow(
      /"availableInNewTerritories" must be a boolean/,
    );
  });
});
...
describe("reconcileAvailability", () => {
  ...
  it("throws when the app has no App Store Connect record", async () => {
    ...
    await expect(reconcileAvailability(api, { bundleId: "com.acme.app", config, dryRun: true })).rejects.toThrow(
  ...
  it("sets the full territory list the first time (no current availability)", async () => {
    ...
    const report = await reconcileAvailability(api, { bundleId: "com.acme.app", config, dryRun: false });
    expect(calls.set).toEqual([{ availableInNewTerritories: false, territories: ["CAN", "GBR", "USA"] }]);
  ...
  it("acts when only the auto-add-new-territories flag changes", async () => {
    ...
    expect(calls.set).toEqual([{ availableInNewTerritories: true, territories: ["CAN", "GBR", "USA"] }]);
  ...
  it("plans but performs nothing on a dry-run", async () => {
    const { api, calls } = makeApi(null);
    const report = await reconcileAvailability(api, { bundleId: "com.acme.app", config, dryRun: true });
    expect(calls.set).toHaveLength(0);
