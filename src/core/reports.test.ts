...
describe("eachDate", () => {
  it("yields every date in an inclusive range", () => {
    expect(eachDate("2026-06-01", "2026-06-03")).toEqual(["2026-06-01", "2026-06-02", "2026-06-03"]);
  });
...
function makeApi(opts: {
  ...
  return {
    ...
    listAnalyticsReports: vi.fn((requestId: string) => Promise.resolve(opts.reports?.[requestId] ?? [])),
    listAnalyticsReportInstances: vi.fn((reportId: string) => Promise.resolve(opts.instances?.[reportId] ?? [])),
    listAnalyticsReportSegments: vi.fn((instanceId: string) => Promise.resolve(opts.segments?.[instanceId] ?? [])),
  };
}
...
describe("ensureAnalyticsRequest", () => {
  ...
  it("skips a request stopped for inactivity and creates a fresh one", async () => {
    const api = makeApi({ requests: [{ id: "stale", accessType: "ONGOING", stoppedDueToInactivity: true }] });
    const { request, created } = await ensureAnalyticsRequest(api, "app1", "ONGOING");
...
describe("collectAnalyticsSegments", () => {
  it("throws an actionable error when the app record is missing", async () => {
    ...
    await expect(
      collectAnalyticsSegments(api, { bundleId: "com.x.missing", accessType: "ONGOING", granularity: "DAILY" }),
