...
describe("reconcileExportCompliance", () => {
  it("does nothing when the app declares no answer", async () => {
    const api = makeApi();
    const result = await reconcileExportCompliance(api, { ...BASE, usesNonExemptEncryption: undefined });
  ...
  it("reports build-not-found when the build hasn't ingested yet", async () => {
    ...
    const result = await reconcileExportCompliance(api, { ...BASE, usesNonExemptEncryption: false });
  ...
  it("is a no-op when the build already carries the desired answer", async () => {
    const api = makeApi({ findBuild: vi.fn().mockResolvedValue({ id: "build1", usesNonExemptEncryption: false }) });
    const result = await reconcileExportCompliance(api, { ...BASE, usesNonExemptEncryption: false });
  ...
  it("flags needs-declaration when no approved declaration exists to reuse", async () => {
    const api = makeApi({
      listEncryptionDeclarations: vi.fn().mockResolvedValue([{ id: "decl-pending", state: "IN_REVIEW" }]),
    });
    const result = await reconcileExportCompliance(api, { ...BASE, usesNonExemptEncryption: true });
...
describe("summarizeExportComplianceResult", () => {
  it("renders a distinct line per outcome", () => {
    ...
    expect(summarizeExportComplianceResult({ status: "build-not-found", buildNumber: 7 })).toContain("7");
    expect(summarizeExportComplianceResult({ status: "already-answered", usesNonExemptEncryption: false })).toContain(
      "already answered",
    );
    expect(summarizeExportComplianceResult({ status: "answered", usesNonExemptEncryption: false })).toContain(
      "answered the encryption question",
    );
    expect(summarizeExportComplianceResult({ status: "reused-declaration", declarationId: "d1" })).toContain("d1");
    expect(summarizeExportComplianceResult({ status: "needs-declaration" })).toContain("App Encryption Declaration");
  });
});
