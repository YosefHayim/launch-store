...
describe("listPlayReviews", () => {
  it("filters by rating and unanswered, client-side", async () => {
    ...
    expect((await listPlayReviews(api, "com.acme.app", { rating: 5 })).map((r) => r.reviewId)).toEqual(["a", "c"]);
    expect((await listPlayReviews(api, "com.acme.app", { unansweredOnly: true })).map((r) => r.reviewId)).toEqual([
      "a",
      "b",
    ]);
  });
...
describe("replyToPlayReview", () => {
  ...
  it("reports replaced=true when a developer reply already existed", async () => {
    const { api } = makeApi([review({ reviewId: "r1", answered: true, developerReply: "Old reply" })]);
