...
const ref = (app: string, profile: string | null, name: string): SecretRef => ({ app, profile, name });
...
describe("effectiveRefs", () => {
  ...
  it("includes only the requested app's secrets", () => {
    expect(effectiveRefs(refs, "acme", "production").map((r) => r.name)).not.toContain("OTHER_TOKEN");
