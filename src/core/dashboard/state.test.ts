...
function app(overrides: Partial<AppDescriptor> = {}): AppDescriptor {
  return { name: "sampleapp", dir: "/apps/sampleapp", configPath: "/apps/sampleapp/app.json", ...overrides };
}

function account(overrides: Partial<AccountRecord> = {}): AccountRecord {
  return { keyId: "KEY1", issuerId: "ISS", label: "Personal", addedAt: NOW.toISOString(), ...overrides };
}
...
describe("buildDashboardState", () => {
  ...
  it("collapses absent app optionals to null", () => {
    const state = buildDashboardState(inputs({ apps: [app({ version: "2.1.0", bundleId: "com.x.y" })] }));
  ...
  it("flags the active account and counts its visible apps", () => {
    const state = buildDashboardState(
      inputs({
        accounts: [account({ keyId: "KEY1", apps: ["a", "b"] }), account({ keyId: "KEY2", label: "Client" })],
        activeKeyId: "KEY2",
      }),
    );
  ...
  it("caps recent artifacts at the limit while preserving newest-first order", () => {
    const many = Array.from({ length: RECENT_ARTIFACT_LIMIT + 5 }, (_, i) => artifact({ buildNumber: i + 1 }));
