...
function state(overrides: Partial<DashboardState> = {}): DashboardState {
  return {
    ...
    project: {
      providers: { credentials: "local", storage: "local", buildEngine: "fastlane", submit: "app-store-connect" },
    ...
    secrets: [{ app: "sampleapp", profile: null, name: "SENTRY_AUTH_TOKEN" }],
    cloudHost: null,
    ...overrides,
  };
}
...
function embeddedJson(html: string): string {
  const match = /<script id="launch-dashboard-state" type="application\/json">([\s\S]*?)<\/script>/.exec(html);
    ...
    const html = renderDashboardHtml(
      state({
        project: {
          providers: { credentials: "local", storage: "local", buildEngine: "fastlane", submit: "app-store-connect" },
          profiles: [],
          apps: [{ name: "<script>alert(1)</script>", version: null, bundleId: null, packageName: null }],
        },
      }),
    );
