...
import type { DocStats } from '@core/types/commandDocs.js';

export type ReadmeMarkerMissing = {
  readonly _tag: 'ReadmeMarkerMissing';
  readonly message: string;
};

export type FaqEntryInvalid = {
  readonly _tag: 'FaqEntryInvalid';
  readonly message: string;
};

const makeReadmeMarkerMissing = Data.tagged<ReadmeMarkerMissing>('ReadmeMarkerMissing');
const makeFaqEntryInvalid = Data.tagged<FaqEntryInvalid>('FaqEntryInvalid');
/**
 ...
 */
export const renderStatsBadges = (stats: DocStats): string => {
...
const spliceRegion = (
  ...
  replacement: string,
  label: string,
): Effect.Effect<string, ReadmeMarkerMissing> => {
  const start = content.indexOf(startMarker);
  let end = -1;
  if (start !== -1) end = content.indexOf(endMarker, start);
  if (start === -1) {
    return Effect.fail(
      makeReadmeMarkerMissing({
        message: `README.md is missing the ${label} markers - add the fences back so \`docs:gen\` can regenerate the ${label} region.`,
      }),
    );
  }
  if (end === -1) {
  ...
  return Effect.succeed(
    content.slice(0, start) + replacement + content.slice(end + endMarker.length),
  );
};
/**
 ...
 */
export const spliceReadmeBadges = (
  readme: string,
  badges: string,
): Effect.Effect<string, ReadmeMarkerMissing> => {
 ...
 */
const parseFaqEntries = (): Effect.Effect<FaqEntry[], FaqEntryInvalid> => {
  return Effect.gen(function* () {
    const faqEntries: FaqEntry[] = [];
    for (const paragraph of GENERATIVE_AI_FAQ.split(/\r?\n\r?\n/)) {
      const match = /^\*\*(.+?)\*\*\s*([\s\S]+)$/.exec(paragraph.trim());
      if (!match?.[1]) {
        return yield* Effect.fail(
          makeFaqEntryInvalid({
            message: `FAQ entry is not in "**Question?** Answer" form: ${paragraph.slice(0, 60)}...`,
          }),
        );
      }
      if (!match[2]) {
      ...
      faqEntries.push({ question: match[1], answer: match[2].trim() });
    }
    return faqEntries;
  });
};
 ...
 */
export const renderCollapsibleFaq = (): Effect.Effect<string, FaqEntryInvalid> => {
  return Effect.map(parseFaqEntries(), (faqEntries) =>
    faqEntries
      .map(({ question, answer }) =>
        [
          '<details>',
 ...
 */
export const renderFaqRegion = (): Effect.Effect<string, FaqEntryInvalid> => {
  return Effect.map(renderCollapsibleFaq(), (faqMarkdown) =>
    [FAQ_REGION_START, '', faqMarkdown, '', FAQ_REGION_END].join('\n'),
  );
};
/**
 ...
 */
export const renderFeaturesList = (): string => {
  let n = 0;
  return FEATURE_SECTIONS.map((section) => {
    const lead: string[] = [];
    if (section.intro) lead.push(section.intro, '');
 ...
 */
export const renderCollapsibleFeatures = (): string => {
  return FEATURE_SECTIONS.map((section) => {
    const lead: string[] = [];
    if (section.intro) lead.push(section.intro, '');
 ...
 */
export const renderFeaturesRegion = (): string => {
 ...
 */
export const spliceReadmeFeatures = (
  readme: string,
  region: string,
): Effect.Effect<string, ReadmeMarkerMissing> => {
 ...
 */
export const renderAgentSkillsRegion = (): string => {
 ...
 */
export const spliceReadmeAgentSkills = (
  readme: string,
  region: string,
): Effect.Effect<string, ReadmeMarkerMissing> => {
