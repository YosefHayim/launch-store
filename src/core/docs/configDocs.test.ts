...
describe("cleanDescription", () => {
  it("strips a bare {@link} down to the symbol's last segment", () => {
    expect(cleanDescription("See {@link ReleaseConfig.earliestReleaseDate}.")).toBe("See earliestReleaseDate.");
  });
  ...
  const schema: JsonSchema = {
    ...
    definitions: {
      LaunchConfigInput: {
        ...
        properties: {
          profiles: { $ref: "#/definitions/Record%3Cstring%2CBuildProfile%3E", description: "Profiles keyed by name." },
          appRoots: {
            type: "array",
            items: { type: "string" },
      ...
      "Record<string,BuildProfile>": { type: "object", additionalProperties: { $ref: "#/definitions/BuildProfile" } },
      BuildProfile: {
        type: "object",
        required: ["name"],
