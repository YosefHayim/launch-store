...
export function renderConfigDocs(schema: JsonSchema): string {
  ...
  const intro = [
    ...
    "Run `launch config schema --out launch.config.schema.json` and add a `$schema` key for editor autocomplete and validation, or `launch config validate` to check a config against this schema.",
  ].join(" ");
