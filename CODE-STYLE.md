...
// ✗ declaration form relies on hoisting
...
// ✗ parses, authenticates, uploads, and renders
...
Production behavior returns Effect while Promise syntax stays in tests, development scripts, generated code, the runtime entrypoint, Commander's callback contract, and Apple's HTTP/JWT transport boundary.
```ts
// ✓ src/core/store/playPricing.ts
...
// ✗ Promise behavior in a core module
...
// ✗ repository wrapper around the same primitive
...
// ✗ generic record guard repeated downstream
...
// ✗ compressed fallback and branching
return releaseInput.mode ?? (releaseInput.canPrompt ? 'prompt' : 'blocked');

// ✗ hiding the same branch in an immediately invoked function
const mode = (() => {
...
// ✗ service class and misplaced screenshot domain table
...
// ✗ transport orchestration in Commander
