...
> Launch wraps **189 App Store Connect & Google Play API operations** across **63 commands**, guarded by **2084 tests**.
...
print the JSON Schema for launch.config.ts
...
validate a config and report each problem with its field path

### `launch config docs`

print the launch.config.ts field reference
...
## `launch creds [action] [firstArgument] [secondArgument]`

inspect credentials, onboard/switch Apple accounts, or provision signing assets

| Flag | Description |
| --- | --- |
| `--platform <platform>` | ios (default), android, tvos, macos, or visionos |
| `--key-id <id>` | Apple Key ID |
| `--issuer-id <id>` | App Store Connect Issuer ID |
| `--p8 <path>` | path to an Apple .p8 key |
| `--label <name>` | human-readable account or key label |
| `--account <name>` | Apple account label or Key ID |
| `-a, --app <name>` | configured app handle |
| `--import <keystore>` | existing Android upload keystore |
| `--alias <alias>` | alias inside an imported Android keystore |
| `--team-id <id>` | Apple Team ID for an APNs key |
| `--out <path>` | APNs export destination |
| `--force` | overwrite an existing APNs export destination |
...
| Flag | Description |
| --- | --- |
| `-a, --app <names>` | comma-separated app handles (default: every applicable app) |
| `--dry-run` | print the plan and make no changes |
| `--allow-destructive` | permit destructive actions |
| `-y, --yes` | skip the confirmation prompt |
| `--no-snapshot` | skip the automatic pre-sync baseline |
...
download App Store Connect sales, finance, and analytics reports

### `launch reports sales`

download a Sales and Trends report (gzipped TSV)

| Flag | Description |
| --- | --- |
| `--vendor-number <n>` | vendor number (or set ASC_VENDOR_NUMBER) |
| `--date <date>` | report date; format follows --frequency |
...
| `--region <code>` | region code: ZZ (all) or a specific region |
| `--report-type <type>` | FINANCE_DETAIL \| FINANCIAL |
| `--out <directory>` | directory to write the report into |
| `--json` | write parsed JSON instead of raw TSV |

### `launch reports analytics`

request and download App Store Connect Analytics reports

| Flag | Description |
| --- | --- |
| `-a, --app <name>` | app handle (auto-selected if there's only one) |
| `--access-type <type>` | ONGOING \| ONE_TIME_SNAPSHOT |
| `--category <category>` | APP_USAGE \| APP_STORE_ENGAGEMENT \| COMMERCE \| FRAMEWORK_USAGE \| PERFORMANCE |
...
| Flag | Description |
| --- | --- |
| `-y, --yes` | confirm teardown without prompting |
...
scaffold CI workflows for building and shipping Launch apps
...
explain a failed native build - parse the cause and fix from a build log
...
| Flag | Description |
| --- | --- |
| `--id <id>` | a build id from `launch builds list` (defaults to latest) |
| `--latest` | re-sign the most recent build |
| `-a, --app <name>` | only consider builds for this app |
| `--account <keyId\|label>` | Apple account whose signing assets should be used |
| `-o, --output <path>` | output file or directory |
| `--dry-run` | print the re-sign plan and change nothing |
