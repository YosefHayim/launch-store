...
- `launch sync` (`core/store/ascSync.ts`) is **push-only and declarative** - config is the source of truth,
...
Apple's OpenAPI spec (`apple/generated/schema.ts`) has **no `appGroups` and no `cloudContainers` endpoint**,
