...
`PlannedAction[]` shape (`src/core/ascSync.ts:103,135`) and `--dry-run` *is* a read-only plan
(`act()`, `src/core/ascSync.ts:185`); and `launch adopt` (ADR 0002) already reads live store state.
...
5. **Polish:** `plan`/`drift` glossary topics for `--explain` (⚠️ bump the `glossary.test.ts`
   `toBe(N)` count — known merge hotspot), `launch plan --json` schema doc, and `npm run docs:gen`
...
`npm run typecheck && npm run lint && npm run test && npm run build` + `format:check` +
...
Plus: `plan`/`drift` glossary topics for each surface's `--explain` (⚠️ bump the `glossary.test.ts`
`toBe(N)` count — known merge hotspot), and `npm run docs:gen` to refresh `docs/commands.md` +
