...
- **One SSOT for install** — a composite action `.github/actions/setup-node-cached` does setup-node +
  the `node_modules` cache (keyed on OS + Node major + exact `package-lock.json` hash, **no**
  `restore-keys`) + `npm ci` on a miss. The cache rule lives in one place instead of being copy-pasted
  into each leg.
