...
The SDK->data-category map, the clause library (keyed **data-category x jurisdiction**), and the
...
src/core/compliance/data/clauseLibrary.ts   // data-category x jurisdiction -> clause text
...
JSDoc on exports, and `pnpm typecheck && pnpm lint && pnpm test && pnpm build` + `format:check`
