...
- `LaunchEnvironmentSchema` decodes `process.env` once and exposes secret fields as `Redacted` values to live layers.
- Official Effect platform services own Node filesystem and path mechanics.
...
- **2026-07-19:** zod removed from the package. Domain types are readonly TypeScript aliases under
  `src/core/types/`; runtime validation and JSON Schema generation both use `LaunchConfigEffectSchema`
  (`scripts/dev/generate-docs.ts` -> `JSONSchema.make`, with `$defs` normalized to draft-07 `definitions`).
