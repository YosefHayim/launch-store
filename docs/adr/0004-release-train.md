...
5. **Polish:** `release-train` glossary topics for `--explain` ([WARN] bump the `glossary.test.ts`
   `toBe(N)` count - known merge hotspot) and `pnpm docs:gen`.

Conventions hold: child processes via `core/exec.ts`, secrets only in the keychain, no `any`/needless
`as`, JSDoc on exports, and `pnpm typecheck && pnpm lint && pnpm test && pnpm build` +
`format:check` + `pnpm docs:check` green before done.
