...
- **App ID (Bundle ID)** — the unique id (`com.example.sampleapp`). Relay registers it via `POST /v1/bundleIds`. → no portal trip.
...
```
relay build ios --profile production [--app sampleapp] [--explain] [--dry-run]
