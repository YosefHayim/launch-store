...
import {
  ...
  normalizeAscSpec,
  pickSpecEntry,
} from '../src/core/asc/specPatch.ts';
import { capture } from '../src/core/exec.ts';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const spec = normalizeAscSpec(
      JSON.parse(await readFile(join(outDir, specEntry), 'utf8')) as OpenAPI3,
    );
    process.stdout.write(
      `Generating types from OpenAPI ${spec.openapi}, spec version ${spec.info.version}\n`,
    );
