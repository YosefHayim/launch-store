...
The technical vocabulary behind Launch: React Native, Expo, Apple, Google Play, signing, build tooling, and distribution. Launch-owned domain language stays in [LANGUAGE.md](./LANGUAGE.md), while the CLI teaching source stays in [src/core/terminal/glossary.ts](./src/core/terminal/glossary.ts).

This guide assumes the reader is new to the mobile-store ecosystem. Run `launch explain <topic>` for the shorter runtime definition of a supported topic.
...
- **App Store Connect** (ASC) - Apple's web portal plus the [App Store Connect API](https://developer.apple.com/documentation/appstoreconnectapi) for apps, builds, TestFlight, and your listing. Launch drives its API for everything scriptable; the few things the API can't do, it points you to the website to finish.
...
- **screenshot specification** - Apple's accepted device slots and pixel dimensions. Launch keeps one registry traced to Apple's [screenshot specifications](https://developer.apple.com/help/app-store-connect/reference/screenshot-specifications).
...
- **bundle id** (App ID) - the app's unique identifier, e.g. `com.example.sampleapp`. Launch registers it over the API before a profile references it.
...
- **Google Play Console** - Google's web portal for publishing Android apps; the Play-side counterpart to App Store Connect. Launch automates releases through the [Google Play Developer API](https://developers.google.com/android-publisher); first-time Play App Signing enrollment happens here.
...
- **Play listing assets** - screenshots are grouped by Play form factor rather than Apple's device display-type enums. Launch validates them against Google's [store listing asset requirements](https://support.google.com/googleplay/android-developer/answer/9866151?hl=en).
...
- **Android vitals** - Google Play's post-launch quality signals (crash rate and ANR rate) that gate your Play ranking and can trigger warnings. `launch play-reports vitals` reads them from the [Play Developer Reporting API](https://developers.google.com/play/developer/reporting) - the Android counterpart to the iOS reports/insights observability.

Launch vendors Apple types from [Apple's published OpenAPI specification](https://developer.apple.com/sample-code/app-store-connect/app-store-connect-openapi-specification.zip). Google adapters use the official generated [`androidpublisher`](https://github.com/googleapis/google-api-nodejs-client/tree/main/src/apis/androidpublisher) and [`playdeveloperreporting`](https://github.com/googleapis/google-api-nodejs-client/tree/main/src/apis/playdeveloperreporting) clients.
