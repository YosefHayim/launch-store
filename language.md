...
- **platform vs store** — two concepts Launch keeps distinct. A **platform** is what you _build_ (`ios`, `android`, `tvos`, `macos`, `visionos`) — the build engine + artifact type. The four Apple platforms build with Xcode and submit to App Store Connect, differing only in the gym destination, the ASC platform attribute, and the signing-profile type (all centralized in `src/core/platform.ts`). A **store** is where you _submit_ (a `Submitter`: App Store Connect, Google Play, and later Amazon Appstore / Galaxy Store / AppGallery). They're no longer 1:1 — one Android build can fan out to several stores via a per-platform `submit` map in `launch.config.ts`. See `docs/adr/0006-platform-store-split.md` and `docs/adr/0007-apple-platform-family.md`.
